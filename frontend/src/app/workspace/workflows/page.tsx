...
export default function WorkflowsPage() {
  ...
  const { workspaces, isLoading, error, refetch } = useTaskWorkspaces();
  ...
  useEffect(() => {
    if (editTaskId && editPrimaryAgent === "" && executableAgents.length > 0) {
      setEditPrimaryAgent(executableAgents[0]!.name);
    }
  }, [editTaskId, editPrimaryAgent, executableAgents]);

  // Also set create wizard primary agent default
  useEffect(() => {
    if (createOpen && primaryAgent === "" && executableAgents.length > 0) {
      setPrimaryAgent(executableAgents[0]!.name);
    }
  }, [createOpen, primaryAgent, executableAgents]);
