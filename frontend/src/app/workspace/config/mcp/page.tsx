...
const MARKITDOWN_PRESET: MCPFormState = {
  name: "markitdown",
  description: "Convert documents to Markdown through the local MarkItDown MCP server.",
  type: "stdio",
  command: "/home/sieve-pub/Vscode-workspace/.venv/bin/markitdown-mcp",
  args: "",
  url: "",
  env: "",
};

const FIRECRAWL_PRESET: MCPFormState = {
  name: "firecrawl",
  description: "Structured web crawling through the official Firecrawl MCP server. Requires FIRECRAWL_API_KEY.",
  type: "stdio",
  command: "npx",
...
export default function MCPConfigPage() {
  ...
  function applyFirecrawlPreset() {
    setEditingServer(null);
    setForm(FIRECRAWL_PRESET);
    setIsFormOpen(true);
  }
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-[1.25rem] border border-border/60 bg-background/40 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Adds the local document-to-Markdown MCP entry backed by the workspace virtual environment.
            </p>
            <p className="mt-1 text-[11px] font-mono text-muted-foreground">
              /home/sieve-pub/Vscode-workspace/.venv/bin/markitdown-mcp
            </p>
          ...
          <div className="rounded-[1.25rem] border border-border/60 bg-background/40 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Adds the official Firecrawl MCP entry for structured crawling. Requires the FIRECRAWL_API_KEY environment variable.
            </p>
            <p className="mt-1 text-[11px] font-mono text-muted-foreground">
              npx -y firecrawl-mcp
            </p>
            <Button size="sm" className="mt-3" variant="outline" disabled={isStatic} onClick={applyFirecrawlPreset}>
              Use preset
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="space-y-1 md:col-span-2">
            ...
            <Textarea
              ...
              placeholder={"FIRECRAWL_API_KEY=$FIRECRAWL_API_KEY"}
              rows={3}
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {servers.map(([name, srv]) => {
            ...
            return (
              <div key={name} className="octo-panel flex flex-col justify-between rounded-[1.5rem] p-4 transition-shadow hover:translate-y-[-1px] hover:shadow-[3px_3px_7px_var(--neu-dark-strong),_-3px_-3px_7px_var(--neu-light-strong)]">
                <div className="mb-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <ServerIcon className="size-4 shrink-0 text-muted-foreground" />
                      <h3 className="min-w-0 break-words text-sm font-medium text-foreground">{name}</h3>
                    </div>
