      ...
      {mounted && (
        <Tabs defaultValue="all" onValueChange={setFilter} className="mb-4">
          <TabsList variant="line">
            <TabsTrigger value="all">{t.sidebar.allPlugins}</TabsTrigger>
            <TabsTrigger value="engineering">{t.sidebar.engineering}</TabsTrigger>
            <TabsTrigger value="review">{t.sidebar.review}</TabsTrigger>
            <TabsTrigger value="runtime">{t.sidebar.runtime}</TabsTrigger>
            <TabsTrigger value="integration">{t.sidebar.integration}</TabsTrigger>
          </TabsList>
        </Tabs>
      )}
              ...
              <div key={plugin.plugin_id} className="octo-panel flex min-w-0 flex-col justify-between rounded-[1.5rem] p-4 transition-shadow hover:translate-y-[-1px] hover:shadow-[3px_3px_7px_var(--neu-dark-strong),_-3px_-3px_7px_var(--neu-light-strong)]">
                <div className="mb-3">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="min-w-0 break-words text-sm font-medium text-foreground">{plugin.display_name}</h3>
                    <div className="flex shrink-0 gap-1">
                      <Button
                        aria-label={`Edit ${plugin.display_name}`}
                        size="icon"
                        variant="ghost"
                        className="size-7"
                        title="Edit"
                        onClick={() => {
                          setInstallId(plugin.plugin_id);
