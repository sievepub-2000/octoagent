...
export function AgentCard({ agent }: AgentCardProps) {
  ...
  const isTemplate = agent.source === "template";
  const isEditable = agent.editable !== false && !isTemplate;
  const isDeletable = agent.deletable !== false && !isTemplate;
  ...
  function handleChat() {
    if (!agent.chat_enabled || isTemplate) {
      const templateKey = agent.template_skill_name && agent.template_id
        ? `${agent.template_skill_name}:${agent.template_id}`
        : "";
      router.push(
        templateKey
          ? `/workspace/agents/new?template=${encodeURIComponent(templateKey)}`
  ...
  const customAvatarUrl = isEditable && agent.avatar
      ...
      >
        <div className="mb-2 flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            <AgentAvatar size={32} avatarUrl={customAvatarUrl} />
            <div className="min-w-0">
              <h2 className="min-w-0 break-words text-sm font-medium text-foreground">
                {displayName}
              </h2>
              {agent.model && (
                <Badge variant="secondary" className="mt-0.5 max-w-full text-[10px]">
                  {agent.model}
                </Badge>
              )}
            </div>
        ...
        {agent.description && (
          <p className="mb-3 break-words line-clamp-2 text-xs text-muted-foreground">
            {agent.description}
          </p>
        )}
