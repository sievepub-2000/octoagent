...
function ToolCall({
  ...
  if (name === "web_search") {
    ...
    // Shell-family tools: always surface the actual command/pattern so the
    // operator can see what the agent is executing. Previously the `bash`
    // branch returned a raw string (not a JSX element) when description was
    // missing, and `host_shell`/`glob`/`grep`/`lsp` fell through to the
    // generic "useTool(name)" label which hid the command entirely.
    const description: string | undefined = (args as { description?: string })
      ?.description;
    const command: string | undefined =
      (args as { command?: string; pattern?: string; query?: string })?.command ??
      (args as { pattern?: string })?.pattern ??
      (args as { query?: string })?.query;
    const cwd: string | undefined = (args as { cwd?: string })?.cwd;
    const label = description ?? command ?? t.toolCalls.executeCommand;
    return (
      <ChainOfThoughtStep key={id} label={label} icon={SquareTerminalIcon}>
        {command && (
          <CodeBlock
