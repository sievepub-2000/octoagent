...
export function TaskWorkspaceRuntime({
  ...
  const orderedWorkspaces = useMemo(
    () => orderWorkspaces(workspaces, linkedTaskIds),
    [linkedTaskIds, workspaces],
  );
    ...
    if (!selectedTaskId) {
      return (
        <Card className="h-full justify-center border-dashed shadow-none">
          <CardHeader>
            <CardTitle>{runtimeCopy.selectRuntimeTitle}</CardTitle>
            <CardDescription>
              {runtimeCopy.selectRuntimeDescription}
            </CardDescription>
          </CardHeader>
        </Card>
      );
  ...
  return (
    <div className="grid h-full min-h-0 gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <div className="min-h-0">{renderWorkspaceDetail()}</div>
    </div>
  );
}