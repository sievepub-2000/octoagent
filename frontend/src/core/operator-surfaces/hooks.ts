...
export function useExecutionDispatches(refetchInterval: number | false = 15000) {
	const { data, isLoading, error, refetch } = useQuery({
		queryKey: ["operator-execution-dispatches"],
		queryFn: loadExecutionDispatches,
		refetchOnWindowFocus: false,
		refetchInterval,
	})
