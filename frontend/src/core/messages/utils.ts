...
/**
 * Strip <client_execution_contract> block from message content.
 * Guards against legacy checkpoints where the contract was
 * injected directly into HumanMessage content.
 */
export function stripContractTag(content: string): string {
  return content
    .replace(/<client_execution_contract>[\s\S]*?<\/client_execution_contract>/g, "")
    .trim();
}
