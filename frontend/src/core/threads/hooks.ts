...
export function useThreadStream({
  ...
  // 30s watchdog — if the server never streams anything back we must not
  // leave the user staring at a forever-spinning optimistic bubble.
  ...
  const sendMessage = useCallback(
    async (
      ...
      // Arm the 30s watchdog.  If the stream never produces a message we
      // surface an error toast and drop the optimistic bubble so the
      // composer re-enables.
      clearWatchdog();
      watchdogRef.current = setTimeout(() => {
        ...
        setOptimisticMessages((prev) => {
          ...
          toast.error("Assistant did not respond within 30s. Please retry.");
          try {
            void thread.stop();
          } catch {
            // ignore — stream may already be settled
          }
          return [];
        });
      }, 30_000);
    ...
    [thread, _handleOnStart, t.uploads.uploadingFiles, context, queryClient, clearWatchdog],
  );
