...
function uniqueWorkflowEvents(events: WorkflowEvent[]) {
  const seen = new Set<string>();
  const uniqueEvents: WorkflowEvent[] = [];
...
export function WorkflowsProvider({
  ...
  const hydrate = useCallback((nextWorkflows: Workflow[], nextEvents: WorkflowEvent[] = []) => {
    ...
    setEvents(uniqueWorkflowEvents(nextEvents).slice(0, 100));
  ...
  const appendEvent = useCallback((event: WorkflowEvent) => {
    setEvents((prev) =>
      uniqueWorkflowEvents([event, ...prev]).slice(0, 100),
    );
  }, []);
