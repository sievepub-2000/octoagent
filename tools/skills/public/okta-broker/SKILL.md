...
This skill turns an account-provisioning brief into a **signed-intent envelope** for Okta. OctoAgent NEVER calls the identity provider directly; output is a structured request payload that an authorized tenant admin executes out-of-band.
...
- `okta_user_create.json` (json, required)
- `okta_group_assignments.json` (json, required)
...
2. **map_to_okta_schema** — transform intake into okta POST /api/v1/users schema; validate required attributes (e.g. SCIM userName, displayName) and emit validation errors before envelope emission
3. **emit_user_create_envelope** — render the okta POST /api/v1/users HTTP request envelope (auth header placeholder, body, expected response shape) — no network call is made
4. **emit_group_assignment_envelope** — render the okta POST /api/v1/groups/{id}/users/{user_id} request envelope for each required role-group; one envelope per group
