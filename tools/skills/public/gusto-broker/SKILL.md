...
This skill turns a hiring brief into a **signed-intent envelope** for the Gusto API. OctoAgent NEVER calls the vendor API; output is a structured request payload (HTTP method, path, headers placeholders, body) that an authorized tenant admin executes out-of-band via a credentialed CLI or curl path.
...
- `gusto_employee_create.json` (json, required)
- `gusto_payroll_setup.json` (json, required)
...
2. **map_to_gusto_schema** — transform intake into gusto API schema fields; mark required vs optional and surface validation errors before envelope emission
3. **emit_employee_create_envelope** — render the gusto employee-create HTTP request envelope (method, path, headers placeholders, JSON/XML body) as a draft artifact — no network call is made
4. **emit_onboarding_assignment_envelope** — render the gusto onboarding/checklist assignment request envelope as a draft artifact
