...
BLOCKED_TRACKED_PATHS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^workspace/default/threads(?:/|$)"),
    re.compile(r"^workspace/env(?:/|$)"),
    re.compile(r"^workspace/runtime(?:/|$)"),
    re.compile(r"^workspace/workflow/task(?:/|$)"),
    re.compile(r"^logs(?:/|$)"),
)
...
def main() -> int:
    ...
    for path in _tracked_files():
        if any(pattern.search(path.as_posix()) for pattern in BLOCKED_TRACKED_PATHS):
            failures.append(f"{path}: runtime state files must not be tracked")
            continue
