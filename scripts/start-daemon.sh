...
# LangChain/OpenAI model clients in this repo reject the SOCKS-style ALL_PROXY
# value present in this environment. Keep HTTP(S) proxy settings intact for
# outbound network access, but drop ALL_PROXY for daemonized app processes.
export ALL_PROXY=""
...
nohup sh -c "cd backend && NO_COLOR=1 $LANGGRAPH_RUN dev --no-browser --allow-blocking --host 127.0.0.1 --port $LANGGRAPH_PORT $LANGGRAPH_EXTRA_FLAGS > ../logs/langgraph.log 2>&1" &
LANGGRAPH_PID=$!
...
nohup sh -c "cd backend && $PY_RUN uvicorn src.gateway.app:app --host 127.0.0.1 --port $GATEWAY_PORT > ../logs/gateway.log 2>&1" &
GATEWAY_PID=$!
...
nohup sh -c "cd frontend && $FRONTEND_CMD > ../logs/frontend.log 2>&1" &
FRONTEND_PID=$!
...
nohup sh -c 'nginx -g "daemon off;" -c "$2" -p "$1" > logs/nginx.log 2>&1' _ "$REPO_ROOT" "$NGINX_CONFIG_RENDERED" &
NGINX_PID=$!
