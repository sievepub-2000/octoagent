...
source "$PROJECT_ROOT/scripts/port-layout.sh"

# Docker Compose command with project name
COMPOSE_CMD="docker compose -p octoagent-dev -f docker-compose-dev.yaml"
...
init() {
    echo "=========================================="
    echo "  OctoAgent Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting OctoAgent Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set OCTO_AGENT_ROOT for provisioner if not already set
    if [ -z "$OCTO_AGENT_ROOT" ]; then
        export OCTO_AGENT_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting OCTO_AGENT_ROOT=$OCTO_AGENT_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting OctoAgent.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  OctoAgent Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    # OCTO_AGENT_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" octoagent-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting OctoAgent Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "OctoAgent Docker Management Script"
    echo ""
