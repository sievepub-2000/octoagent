#!/usr/bin/env bash
# chat.sh — Send a message to OctoAgent and collect the streaming response.
#
# Usage:
...
#   OCTOPUSAGENT_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   OCTOPUSAGENT_URL          — Unified proxy base URL (default: http://localhost:19880)
#   OCTOPUSAGENT_GATEWAY_URL  — Gateway API base URL (default: $OCTOPUSAGENT_URL)
#   OCTOPUSAGENT_LANGGRAPH_URL — LangGraph API base URL (default: $OCTOPUSAGENT_URL/api/langgraph)
#
...
GATEWAY_URL="${OCTOPUSAGENT_GATEWAY_URL:-$OCTOPUSAGENT_URL}"
LANGGRAPH_URL="${OCTOPUSAGENT_LANGGRAPH_URL:-$OCTOPUSAGENT_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: OctoAgent is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure OctoAgent is running. Start it with: cd <octoagent-dir> && make dev" >&2
  exit 1
fi
...
gateway_url = sys.argv[2].rstrip("/") if len(sys.argv) > 2 else "http://localhost:19880"
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from OctoAgent: {data_str}", file=sys.stderr)
            sys.exit(1)
