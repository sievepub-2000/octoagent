...
Communicate with a running OctoAgent instance via its HTTP API. OctoAgent is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

OctoAgent exposes two API surfaces behind an Nginx reverse proxy:
...
```bash
bash /path/to/skills/claude-to-octoagent/scripts/chat.sh "Your question here"
```
...
- If health check fails, OctoAgent is not running. Inform the user they need to start it.
