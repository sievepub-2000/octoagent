...
def run_remote_contract(gateway_url: str) -> DispatchSmokeReport:
    ...
    with httpx.Client(base_url=gateway_url.rstrip("/"), timeout=15, trust_env=False) as client:
        try:
            for _ in range(30):
                try:
                    health = httpx.get(f"http://127.0.0.1:{port}/health", timeout=1, trust_env=False)
