...
AGENT_DEFINITIONS: dict[str, dict[str, str]] = {
    ...
    "weather-coordinator": {
        ...
        "soul": (
            "You are a coordinator agent for weather workflows. "
            "You must decompose tasks, rely on sub-agents for each city, and synthesize only from their real outputs. "
    ...
    "speed-orchestrator": {
        ...
        "soul": (
            "You are the lead coordinator for inference-performance investigations. "
            "You must assign sub-agents, compare their findings, and deliver a concrete optimization plan only from their real outputs."
        ),
    },
...
SCENARIOS: list[ScenarioDefinition] = [
    ScenarioDefinition(
        ...
        mode="single",
        goal=(
            "Use the latest public web information to report Kyoto, Osaka, and Tokyo weather for the next 3 days. "
            "Include date, condition, max/min temperature in Celsius, precipitation probability, and source URLs for each city."
        ),
    ...
    ScenarioDefinition(
        ...
        mode="branch",
        goal=(
            "The coordinator must ask three sub-agents to independently gather the latest public 3-day weather forecasts for Tokyo, Osaka, and Kyoto, then synthesize the combined result with source URLs."
        ),
...
def create_workspace_via_webui(page: Page, definition: ScenarioDefinition) -> dict[str, Any]:
    ...
    if definition.mode != "single":
        topology_pattern = (
            re.compile(r"Branch|分支|ブランチ|브랜치", re.I)
            if definition.mode == "branch"
            else re.compile(r"Swarm|群组|群組|群集|スウォーム|스웜", re.I)
        )
...
def collect_workspace_evidence(task_id: str) -> dict[str, Any]:
    ...
    for agent in agents_payload.get("agents", []):
        agent_messages[agent["name"]] = api_request(
            f"/api/task-workspaces/{task_id}/agents/{agent['agent_id']}/messages"
        )
    return {
        "workspace": workspace,
        "cards": cards,
...
def validate_scenario(definition: ScenarioDefinition, evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    non_system_assistants = [
        message for message in assistant_messages if "initialized for task workspace orchestration" not in message["content"]
    ]
    ...
    missing_keywords = [
        keyword for keyword in definition.expected_keywords if keyword.lower() not in result_content.lower()
    ]
    agent_names = [agent.get("name") for agent in evidence["agents"].get("agents", [])]
    incomplete_agents = [
        agent.get("name")
        for agent in evidence["agents"].get("agents", [])
        if agent.get("status") not in {"completed", "running"}
    ]
    pass_checks = {
...
def main() -> int:
    ...
    report["overall_status"] = (
        "passed"
        if all(item["validation"]["status"] == "passed" for item in report["scenarios"])
        else "failed"
    )
