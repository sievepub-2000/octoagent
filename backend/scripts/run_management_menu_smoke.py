...
def _browser_launch_options(playwright) -> dict[str, object]:
    ...
    executable = next((candidate for candidate in candidates if candidate and os.path.exists(candidate)), None)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=__doc__,
        epilog="Local user setup requires OCTO_AUTH_DEV_EXPOSE_CODES=1 so the smoke can read the registration code without SMTP.",
    )
...
def _create_user() -> tuple[str, str, str, str]:
    os.environ.setdefault("OCTO_AUTH_DEV_EXPOSE_CODES", "1")
...
def main() -> int:
    ...
    routes = [
        ...
        "/workspace/chats/new?settings=mcp&mock=true",
        "/workspace/chats/new?settings=permissions&mock=true",
