...
from src.optimization_program import OptimizationProgram, get_optimization_program
...
def _coverage_summary(repo_root: Path, program: OptimizationProgram) -> dict[str, Any]:
    declared_paths = {
        owned_path
        for area in program.coverage_areas
        for owned_path in area.owned_paths
        if not owned_path.endswith(".py") and not owned_path.endswith(".js")
    }
...
def _runtime_truth_score(repo_root: Path) -> tuple[int, list[dict[str, Any]]]:
    ...
    controller_contract_ready = (
        "TaskWorkspaceExecutionController" in execution_text
        and "TaskWorkspaceMessageExecutor" in execution_text
        and "TaskWorkspaceExecutionController" in workflow_service_text
        and "TaskWorkspaceMessageExecutor" in workflow_service_text
    )
    checks.append(
        {
...
def _capability_governance_score(repo_root: Path) -> tuple[int, list[dict[str, Any]]]:
    ...
    core_dirs = all(
        _path_exists(repo_root, relative)
        for relative in ["backend/src/capability_core", "backend/src/hook_core"]
    )
...
def _format_text(report: dict[str, Any]) -> str:
    ...
    for dimension in report["dimensions"]:
        lines.append(
            f"- {dimension['dimension_id']}: {dimension['current_score']}/{dimension['weight']} (target {dimension['target_score']})"
        )
    lines.extend(
        [
            "",
