...
def _wait_for_uploaded_file(
    ...
    with httpx.Client(timeout=timeout_seconds, trust_env=False) as client:
        while time.monotonic() < deadline:
            uploaded = client.get(f"{gateway_url}/api/threads/{thread_id}/uploads/list")
            uploaded.raise_for_status()
            uploaded_files = uploaded.json().get("files") or []
            last_files = [item for item in uploaded_files if isinstance(item, dict)]
            if any(item.get("filename") == filename for item in last_files):
                return last_files
            time.sleep(0.5)
    return last_files
...
def _complete_setup_wizard_if_present(page) -> None:
    ...
    # New single-step wizard: preserve the backend-configured default model.
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        uploaded_files = _wait_for_uploaded_file(
            args.gateway_url,
            current_thread_id,
            smoke_attachment_name,
            args.timeout_seconds,
        )
