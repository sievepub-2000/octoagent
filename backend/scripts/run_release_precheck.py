...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--frontend-url",
        default="http://127.0.0.1:19880",
        help="Frontend URL used by smoke test (default: http://127.0.0.1:19880).",
    )
    parser.add_argument(
        "--gateway-url",
        default="http://127.0.0.1:19880",
        help="Gateway URL used by smoke test (default: http://127.0.0.1:19880).",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=float,
...
def main() -> int:
    ...
    steps.append(
        _run_step(
            name="legacy runtime path hygiene",
            command=[str(python_bin), "../scripts/check_legacy_paths.py"],
            cwd=backend_root,
        )
    )
