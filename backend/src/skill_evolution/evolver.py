...
from src.skill_evolution.analyzer import AnalysisSuggestion
from src.skill_evolution.registry import SkillEvolutionRegistry
from src.skill_evolution.types import EvolutionConfig, EvolutionMode
...
class SkillEvolver:
    ...
    def _apply_captured(self, name: str, suggestion: AnalysisSuggestion) -> None:
        ...
        content = (
            "---\n"
            f"name: {name}\n"
            f"description: Auto-captured skill — {_sanitize(suggestion.reason[:100])}\n"
            "---\n\n"
            f"# {name}\n\n"
            f"Captured from successful execution.\n\n"
            f"**Reason:** {_sanitize(suggestion.reason)}\n"
        )
