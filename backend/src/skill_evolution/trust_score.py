...
- Guarded by ``SKILL_TRUST_OBSERVATION_ENABLED`` env var (default ``"0"``) so
  it is a pure no-op unless an operator opts in.
...
_DEFAULT_LEDGER = Path("workspace/skill_evolution/trust_scores.jsonl")


def is_enabled() -> bool:
    return os.environ.get(_ENV_FLAG, "0").strip() not in {"", "0", "false", "False", "off", "no"}
