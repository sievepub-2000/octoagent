...
class EvaluationRunner:
    ...
    async def run(self, config: EvalJobConfig) -> EvalJobResult:
        ...
        async def bounded(sample: EvalSample) -> SampleResult:
            async with sem:
                return await self._evaluate_sample(
                    sample, config.dimensions, config.timeout_per_sample_s
                )

        sample_results: list[SampleResult] = await asyncio.gather(
            *[bounded(s) for s in config.samples]
        )
        ...
        logger.info(
            f"Eval job '{config.job_id}' complete: "
            f"{len(successful)}/{len(sample_results)} OK, "
            f"avg_overall={avg_str}"
        )
        return result
