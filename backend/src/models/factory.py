...
def _select_default_model_name(
    *,
    ...
    configured_default = resolve_configured_default_model_name(
        model.name for model in config.models
    )
    if configured_default:
        configured_model = config.get_model_config(configured_default)
        if configured_model and _supports_required_capabilities(
            configured_model,
            thinking_enabled=thinking_enabled,
            requires_vision=requires_vision,
            min_context_tokens=min_context_tokens,
        ):
            if not thinking_enabled or configured_model.supports_thinking:
