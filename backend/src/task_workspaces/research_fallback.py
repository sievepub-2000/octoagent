...
def _fetch_daily_forecast(location: dict[str, Any], days: int) -> tuple[dict[str, Any] | None, str]:
    ...
    params = (
        ...
        f"&forecast_days={days}&timezone={urllib.parse.quote(tz)}"
    )
...
def build_server_side_research_fallback(query: str) -> str:
    ...
    try:
        ...
        if required_domain and not _output_contains_required_domain(search_output, required_domain):
            return (
                "Server-side research fallback failed because required domain evidence was not retrieved. "
                f"Required domain: {required_domain}. Query: {query}. Search output: {search_output}"
            )
        return (
            "Server-side research fallback collected public web results because the model produced no tool calls.\n\n"
        ...
        return (
            ...
            f"Please retry task execution. Details: {exc}"
        )
...
def _fetch_ai_news_rss_source(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in root.findall(".//item"):
        ...
        if (
            entry["title"]
            and entry["link"]
            and _entry_link_matches_source(entry, source)
            and _entry_matches_ai_model_news(entry)
        ):
