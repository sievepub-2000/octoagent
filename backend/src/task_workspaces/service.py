...
from src.agent_core.lifecycle import AgentLifecycleFacade
from src.agent_runtime import get_agent_runtime_manager
from src.brain import BrainCoreService
from src.orchestration import get_orchestration_service
from src.plugins import get_plugin_service
from src.query_engine import QuerySession, get_query_engine_service
from src.research_runtime import get_research_runtime_service
from src.workflow_core.projection import WorkflowProjectionFacade
...
class TaskWorkspaceService:
    ...
    def _agent_card_permission_mode(
        ...
        if agent is not None and agent.linked_card_id is not None:
            ...
            if linked_card is not None:
                return linked_card.permission_mode
        mode = str(workspace.metadata.get("default_permission_mode") or "workspace")
        return mode if mode in {"workspace", "system", "yolo"} else "workspace"
    ...
    def _workflow_metadata_from_summary(cls, summary: str | None) -> dict[str, object]:
        ...
        primary_agent = (
            primary_agent_candidate.strip()
            if isinstance(primary_agent_candidate, str) and primary_agent_candidate.strip()
            else None
        )
        ...
        scheduled_at = (
            scheduled_at_candidate.strip()
            if isinstance(scheduled_at_candidate, str) and scheduled_at_candidate.strip()
            else None
        )
    ...
    def create_workspace(self, request: CreateTaskWorkspaceRequest) -> TaskWorkspace:
        ...
        wizard_primary_agent = (
            str(workflow_summary_metadata.get("workflow_primary_agent"))
            if isinstance(workflow_summary_metadata.get("workflow_primary_agent"), str)
            else None
        )
        wizard_sub_agents = (
            [item for item in workflow_summary_metadata.get("workflow_sub_agents", []) if isinstance(item, str)]
            if isinstance(workflow_summary_metadata.get("workflow_sub_agents"), list)
            else None
        )
        wizard_topology = (
            str(workflow_summary_metadata.get("workflow_topology"))
            if isinstance(workflow_summary_metadata.get("workflow_topology"), str)
            else None
        )
        ...
        workspace = TaskWorkspace(
            ...
            metadata={
                ...
                "coordination_strategy": (
                    ...
                    else ("manager_review" if request.mode == "group" else "solo")
                ),
