...
class TaskWorkspaceBlueprintFactory:
    ...
    def rebuild_agents(
        ...
        *,
        ...
        for index, blueprint in enumerate(blueprints):
            ...
            conversation = (
                matched_agent.conversation.model_copy(update={"task_id": task_id, "agent_id": agent_id})
                if matched_agent is not None
                else AgentConversationRef(task_id=task_id, agent_id=agent_id)
            )
            agents.append(
                AgentHandle(
                    agent_id=agent_id,
    ...
    def make_agents(
        ...
        *,
        ...
        for blueprint in blueprints:
            ...
            metadata["langgraph_thread_scope"] = (
                "agent" if mode in {"branch", "group"} else "workspace"
            )
    ...
    def _agent_blueprints_from_wizard(
        ...
        primary_scope = (
            "Planning, dispatching, supervising, and reporting"
            if mode in ("branch", "group")
            else "Single-chain execution"
        )
