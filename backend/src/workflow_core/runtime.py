...
from src.agent_runtime import AgentExecutionRequest, get_agent_runtime_manager
...
def _resolve_langgraph_assistant_id(workspace: TaskWorkspace | None, agent: Any | None) -> str:
    ...
    return (
        _runtime_hint(agent_metadata.get("langgraph_assistant_id"))
        or _runtime_hint(workspace_metadata.get("langgraph_assistant_id"))
        or "lead_agent"
    )
...
def _resolve_langgraph_graph_id(workspace: TaskWorkspace | None) -> str | None:
    if workspace is None or not isinstance(workspace.metadata, dict):
        return None
    return (
        _runtime_hint(workspace.metadata.get("langgraph_graph_id"))
        or _runtime_hint(workspace.metadata.get("compiled_graph_id"))
    )
...
def recoverable_orphaned_workspaces() -> list[TaskWorkspace]:
    ...
    return [
        ...
        if workspace.status == "running" and not has_agent_messages(workspace)
    ]
