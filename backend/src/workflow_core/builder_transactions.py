...
from src.brain.contracts import BrainBuilderActionModel, BrainTaskContext
from src.brain.service import BrainCoreService
from src.task_workspaces.contracts import TaskWorkspace
...
class BuilderTransactionService:
                    ...
                    for key in action.patch:
                        if key in current_draft and current_draft[key] != action.patch[key]:
                            conflict_warnings.append(
                                f"Action '{action.id}' modifies '{key}' which already has a draft value"
                            )
    ...
    def apply_batch(
        ...
        *,
        ...
        selected_actions = (
            auto_actions
            if use_apply_all_patch or not action_ids
            else [action_index[aid] for aid in action_ids if aid in action_index]
        )
