...
from src.agent_core.session import build_agent_runtime_summary, build_handoff_summary
...
from src.task_workspaces import (
...
from src.task_workspaces.workflow_module import TaskWorkflowModule
from src.workflow_core.builder_transactions import (
...
from src.workflow_core.public_runtime_contracts import (
    PublicBindings,
...
from src.workflow_core.runtime_contracts import TaskStudioRuntimeEventsResponse, TaskStudioRuntimeResponse
from src.workflow_core.status import workflow_stage_for_status
from src.workflow_core.studio_runtime_builder import build_normalized_runtime
...
class WorkflowCoreService:
    ...
    def get_studio_runtime_contract(self, task_id: str) -> dict[str, object] | None:
        ...
        query_sessions = [
            session
            for session in get_query_engine_service().list_sessions()
        ...
        enabled_binding_count = sum(
            1
            for binding_group in bindings.values()
            for binding in binding_group
            if bool(binding.get("enabled"))
        )
        raw_contract = {
            ...
            "workflow_summary": {
                ...
                "active_cards": len([
                    card for card in workspace.card_graph.cards if card.status in {"running", "paused", "blocked"}
                ]),
    ...
    def get_public_runtime(self, task_id: str) -> dict[str, object] | None:
        """Return a stable public runtime snapshot for external consumption.

        This is a narrower view than the internal studio-runtime contract,
        designed for SDK / widget / channel integrations.
        """
        contract = self.get_studio_runtime_contract(task_id)
        if contract is None:
            return None
        normalized = build_normalized_runtime(contract)
        return PublicWorkflowRuntime.model_validate(_project_public_runtime(normalized)).model_dump(mode="json")

    def get_public_runtime_events(
        self,
        task_id: str,
        *,
        cursor: int = 0,
        ...
        """Return paginated public timeline events."""
        result = self.list_studio_runtime_events(task_id, cursor=cursor, limit=limit)
        if result is None:
            return None
        return PublicWorkflowEvents.model_validate({
            "workflow_id": task_id,
    ...
    def update_public_bindings(
        ...
        *,
        ...
        if channels is not None:
            metadata["channel_bindings"] = [
                {"kind": ch, "label": ch, "enabled": True, "status": "enabled"}
                for ch in channels
        ...
        self._workspace_store.update_workspace(
            task_id,
            metadata=metadata,
        )
        return self.get_public_bindings(task_id)

    def get_public_artifacts(self, task_id: str) -> list[dict[str, object]] | None:
        """Return artifact listing for external consumption."""
        workspace = self.get_workspace(task_id)
        if workspace is None:
            return None
        artifacts = self.list_artifacts(task_id)
        return [
            {
                ...
                "download_url": f"/api/task-workspaces/{task_id}/artifacts/{artifact.get('path', '')}?download=true",
            }
            for artifact in artifacts
    ...
    def apply_builder_action(
        ...
        response, new_state = self._builder_transactions.apply_action(
            workspace, preview, action_id
        )
