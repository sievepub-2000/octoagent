...
class OctoAgentClient:
    ...
    async def connect(self, subscriptions: set[str] | None = None) -> None:
        ...
        try:
            ...
            raise RuntimeError("Install 'websockets' package to use the OctoAgent SDK client: pip install websockets") from exc
        ...
        if subscriptions:
            await self._ws.send(json.dumps({"action": "subscribe", "events": sorted(subscriptions)}))
