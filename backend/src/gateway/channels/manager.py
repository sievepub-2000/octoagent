"""ChannelManager — consumes inbound messages and dispatches them to the OctoAgent agent via LangGraph Server."""
...
from src.gateway.channels.message_bus import InboundMessage, InboundMessageType, MessageBus, OutboundMessage, ResolvedAttachment
...
# Per-host hardware-aware default (large host: 1e6, small: 1e4, tiny: 1e3).
from src.agents.resource_profile import get_resource_profile  # noqa: E402

DEFAULT_RUN_CONFIG: dict[str, Any] = {"recursion_limit": get_resource_profile().recursion_default}
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the OctoAgent agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        run_config, run_context = normalize_remote_run_payload(
            run_config,
            run_context,
        )
    ...
    async def _handle_chat(self, msg: InboundMessage) -> None:
        ...
        # Look up existing OctoAgent thread by topic_id (if present)
        thread_id = None
        if msg.topic_id:
