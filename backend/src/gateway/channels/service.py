...
from src.gateway.channels.manager import ChannelManager
from src.gateway.channels.message_bus import MessageBus
from src.gateway.channels.store import ChannelStore
...
def _bridge_fields() -> list[dict[str, Any]]:
    return [
        ...
        _field(
            ...
            "Login Status URL",
            "url",
            description="Optional bridge endpoint that returns the logged-in account identity.",
        ),
        _field(
            "logout_url",
            "Logout URL",
            "url",
            description="Optional bridge endpoint used to sign out or stop the upstream channel session.",
        ),
        _field("qrcode_path", "QR Code Path", "text", description="Optional local path to a bridge login QR image."),
    ]
...
_CHANNEL_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "telegram": {
        "import_path": "src.gateway.channels.telegram:TelegramChannel",
        "handler_path": "src.gateway.channels.telegram:TelegramChannel",
    ...
    "qq": {
        "import_path": "src.gateway.channels.external_bridge:ExternalBridgeChannel",
        "handler_path": "src.gateway.channels.external_bridge:ExternalBridgeChannel",
        ...
        "bridge_project": "NapNeko/NapCatQQ",
        "bridge_project_url": "https://github.com/NapNeko/NapCatQQ",
...
class ChannelService:
    ...
    def __init__(self, channels_config: dict[str, Any] | None = None) -> None:
        ...
        langgraph_url = config.pop("langgraph_url", None) or "http://localhost:19804"
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        for name, registry_entry in _CHANNEL_REGISTRY.items():
            ...
            channels_status[name] = {
                ...
                "ingest_path": f"/api/channels/{name}/ingest" if registry_entry.get("integration_mode") == "external_bridge" else None,
                "outbound_configured": bool(isinstance(config, dict) and str(config.get("outbound_url") or "").strip()),
                "logout_supported": callable(getattr(channel, "logout", None)) if channel is not None else registry_entry.get("integration_mode") == "external_bridge",
                "identity_supported": callable(getattr(channel, "get_auth_status", None)) if channel is not None else registry_entry.get("integration_mode") == "external_bridge",
            }
        return {
    ...
    async def logout_channel(self, name: str) -> dict[str, Any]:
        """Logout a running channel account when supported."""
        channel = self._channels.get(name)
        logout = getattr(channel, "logout", None) if channel is not None else None
        if callable(logout):
            return await logout()
        return {"success": True, "message": "Local channel configuration cleared; channel was not running."}
