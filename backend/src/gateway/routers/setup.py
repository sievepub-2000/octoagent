...
class UpdateDefaultModelRequest(BaseModel):
    default_model: str = Field(..., min_length=1, description="Preferred model name")


class UpdateDefaultModelResponse(BaseModel):
    success: bool
    workspace_path: str = ""
    default_model: str = ""
    sandbox_mode: str = "local"
    ...
    *,
    ...
    setup_snapshot = {
        ...
        "layout": {
            ...
            "runtime_dir": str(workspace_root / "runtime"),
            "checkpoints_path": str(workspace_root / "runtime" / "checkpoints.db"),
            "system_memory_path": str(workspace_root / "runtime" / "system_memory.duckdb"),
            "global_memory_path": str(workspace_root / "default" / "global_memory.json"),
        },
...
def _update_workspace_env_default_model(workspace_root: Path, *, default_model: str) -> None:
    setup_file = workspace_root / "env" / "setup.json"
    if not setup_file.exists():
        _save_workspace_env_state(
            workspace_root,
            default_model=default_model,
            sandbox_mode=_load_setup_state().get("sandbox_mode", "local"),
            directories_created=[],
        )
        return
    try:
...
@router.post(
    "/default-model",
    response_model=UpdateDefaultModelResponse,
    summary="Update the configured default chat model",
)
async def update_default_model(req: UpdateDefaultModelRequest) -> UpdateDefaultModelResponse:
    """Persist the system default model without reapplying workspace setup."""
    model_name = req.default_model.strip()
    app_config = get_app_config()
    if app_config.get_model_config(model_name) is None:
        return UpdateDefaultModelResponse(
            success=False,
    ...
    workspace = setup_state.get("workspace_path", "").strip() or str(get_paths().base_dir)
    sandbox_mode = setup_state.get("sandbox_mode", "local").strip() or "local"

    try:
        resolved = _resolve_path(workspace)
    except Exception as e:
        return UpdateDefaultModelResponse(success=False, error=f"Invalid path: {e}")
    ...
    try:
        ...
        _update_workspace_env_default_model(resolved, default_model=model_name)
    except PermissionError:
        return UpdateDefaultModelResponse(
            success=False,
            workspace_path=str(resolved),
            default_model=model_name,
            sandbox_mode=sandbox_mode,
            error="Permission denied when updating default model",
        )
    except OSError as e:
        return UpdateDefaultModelResponse(
            success=False,
            ...
            default_model=model_name,
            sandbox_mode=sandbox_mode,
            error=f"Failed to update default model: {e}",
        )

    logger.info("Default model updated: workspace=%s, model=%s", resolved, model_name)
    return UpdateDefaultModelResponse(
        success=True,
        workspace_path=str(resolved),
        default_model=model_name,
