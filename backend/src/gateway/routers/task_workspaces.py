...
from src.multi_tenant import get_tenant_registry
...
def _extract_schedule_state(workspace: TaskWorkspace) -> tuple[str | None, str | None, bool]:
    ...
    run_mode = (
        str(run_mode_candidate).strip().lower()
        if isinstance(run_mode_candidate, str)
        else None
    )
    ...
    schedule_at = (
        str(schedule_at_candidate).strip()
        if isinstance(schedule_at_candidate, str) and str(schedule_at_candidate).strip()
        else None
    )
...
async def list_task_workspaces(
    ...
    if x_tenant_id:
        ...
        workspaces = [
            ...
            if str((workspace.metadata or {}).get("tenant_id") or "default") == tenant_id
        ]
...
async def create_task_workspace(
    ...
    current_count = sum(
        1
        for workspace in get_workflow_core_service().list_workspaces()
        if str((workspace.metadata or {}).get("tenant_id") or "default") == tenant_id
    )
    if not tenant_registry.enforce_workspace_limit(tenant_id, current_count):
        ...
        raise HTTPException(
            status_code=409,
            detail=(
                f"Tenant '{tenant_id}' workspace limit exceeded "
                f"({current_count}/{policy.max_concurrent_workspaces})"
            ),
        )
    workspace = get_workflow_core_service().create_workspace(request)
    workspace = get_workflow_core_service().merge_workspace_metadata(
        workspace.task_id,
        tenant_id=tenant_id,
...
async def get_task_workspace_result(task_id: str) -> TaskResultResponse:
    ...
    if hasattr(service, "read_result_payload"):
        ...
        return TaskResultResponse(
            ...
            available_sources=[
                str(item)
                for item in payload.get("available_sources", [])
