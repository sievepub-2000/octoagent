...
async def upload_files(
    ...
    sandbox_id = await sandbox_provider.acquire_async(thread_id)
    sandbox = await sandbox_provider.get_async(sandbox_id)
    ...
    for file in files:
        ...
        try:
            ...
            if sandbox_id != "local":
                if sandbox is None:
                    raise RuntimeError("Sandbox not found after acquisition")
            ...
            if file_ext in CONVERTIBLE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    if sandbox_id != "local":
                        if sandbox is None:
                            raise RuntimeError("Sandbox not found after acquisition")
                        md_content = await asyncio.to_thread(md_path.read_bytes)
                        await asyncio.to_thread(sandbox.update_file, md_virtual_path, md_content)
