...
async def create_tenant(
    ...
    from src.multi_tenant import TenantContext, TenantIsolationPolicy
...
async def update_tenant_policy(
    ...
    from src.multi_tenant import TenantIsolationPolicy
...
async def resolve_tenant(
    ...
    """Resolve the current request's tenant from the X-Tenant-ID header.

    Returns the 'default' tenant if no header is present.
    """
    reg = get_tenant_registry()
    tenant_id = _resolve_tenant_id(x_tenant_id)
    tenant = reg.get_tenant(tenant_id)
    if x_tenant_id and tenant.tenant_id == "default" and tenant_id != "default":
        raise HTTPException(status_code=404, detail=f"Tenant '{tenant_id}' not found")
    resolved_from = "header" if x_tenant_id else "default"
