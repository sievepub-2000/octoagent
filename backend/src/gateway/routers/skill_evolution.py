...
from src.storage.skill_evolution.quality_monitor import QualityMonitor
from src.storage.skill_evolution.registry import SkillEvolutionRegistry
from src.storage.skill_evolution.trust_score import is_enabled as _trust_enabled
from src.storage.skill_evolution.trust_score import summarize_scores as _trust_summarize
from src.storage.skill_evolution.types import EvolutionConfig, EvolutionRecord, QualityMetrics, SkillVersion
...
async def get_trust_scores(window: int | None = None) -> TrustScoreSummaryResponse:
    ...
    entries = [TrustScoreEntry(skill=skill, **data) for skill, data in sorted(summary.items())]
