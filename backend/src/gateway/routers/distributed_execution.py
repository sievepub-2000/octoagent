...
async def register_node(
    ...
    from src.interfaces.distributed import ExecutionNode
...
async def deregister_node(
    ...
    if not confirmation_matches("REMOVE NODE", x_octoagent_confirmation):
        raise HTTPException(status_code=409, detail="Node removal requires confirmation: CONFIRM REMOVE NODE")
    reg = get_execution_node_registry()
