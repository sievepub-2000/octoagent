...
def _estimate_tokens(text: str) -> int:
    ...
    Roughly 1 token per 4 characters for English text, adjusted
    upward for code/CJK content.
    """
    if not text:
        return 0
    ascii_count = sum(1 for c in text if ord(c) < 128)
    non_ascii = len(text) - ascii_count
    # ASCII chars ~4 per token, CJK/symbol ~1.5 per token
...
class CompactionConfig(BaseModel):
    ...
    strategy: str = Field(
        default="truncate",
        description="Compaction strategy: 'truncate' (drop old), 'summarize' (LLM summary), 'hybrid'",
    )
    ...
    def compact(self, messages: list[Message]) -> CompactionResult:
        ...
        if middle_msgs:
            if self._config.strategy == "truncate":
                # Drop middle messages entirely — cheapest strategy
                summary_msg = Message(
                    role="system",
                    content=f"[Session compaction — {len(middle_msgs)} older messages truncated]",
                    token_count=10,
                    is_system=True,
                )
            else:
                ...
                summary_msg = Message(
                    role="system",
                    content=f"[Session compaction — {len(middle_msgs)} messages summarised]\n{summary_text}",
        ...
        if self._llm_summarize is not None:
            try:
                prompt = (
                    ...
                    "Preserve key decisions, action items, and important context.\n\n"
                    f"{transcript}"
                )
