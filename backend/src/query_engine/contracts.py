...
from src.agent_runtime.contracts import AgentRuntimeProviderName
...
class QueryEngineCapability(BaseModel):
    ...
    note: str = (
        "Query engine owns session-scoped turn execution between workspace/orchestration "
        "state and provider-facing query loops."
    )
...
class QueryOperationPlanRequest(BaseModel):
    ...
    permission_mode: Literal["workspace", "system", "yolo"] = "workspace"
