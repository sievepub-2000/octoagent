...
from src.browser_runtime import get_browser_runtime_service
from src.config.extensions_config import ExtensionsConfig
from src.config.paths import get_paths
from src.ml_intern_defaults import build_ml_intern_runtime_context
...
class QueryEngineService:
    ...
    def latest_session_for_agent(self, task_id: str, agent_id: str) -> QuerySession | None:
        candidates = [
            session
            for session in self._sessions.values()
            if session.task_id == task_id and session.agent_id == agent_id
        ]
        if not candidates:
            return None
    ...
    def create_workspace_session(
        ...
        *,
        ...
        next_index = (
            len([session for session in self._sessions.values() if session.task_id == workspace.task_id and session.agent_id == agent.agent_id])
            + 1
        )
    ...
    def execute_turn(
        ...
        *,
        ...
        allow_side_effects = request.allow_side_effects or str(session.metadata.get("permission_mode")) in {"system", "yolo"}
        ...
        self._append_event(
            ...
            detail=(
                f"Client operation '{client_command.operation_id}' mapped to "
                f"intent '{client_command.intent}' and target '{client_command.execution_target}'."
            ),
            created_at=created_at,
        )
        ...
        if approval_required and not allow_side_effects:
            assistant_summary = (
                f"Execution target '{target}' requires approval. "
                "Turn recorded without side effects; enable approved execution to continue."
            )
    ...
    def mark_session_running(
        ...
        *,
        ...
        self._append_event(
            ...
            kind="client_command_planned",
            detail=(
                f"Agent execution started for operation '{client_command.operation_id}' "
                f"against target '{client_command.execution_target}'."
            ),
            created_at=created_at,
        )
        session.governance = self._build_governance(
    ...
    def _agent_permission_mode(self, workspace, agent) -> str:
        ...
        if linked_card is not None:
            mode = getattr(linked_card, "permission_mode", "workspace")
        ...
        mode = str(agent_metadata.get("permission_mode") or workspace_metadata.get("default_permission_mode") or "workspace")
        return mode if mode in {"workspace", "system", "yolo"} else "workspace"
