...
class ModelConfig(BaseModel):
    ...
    use: str | None = Field(
        default=None,
        description=(
            "Optional class path of the model provider (for example: langchain_openai:ChatOpenAI). "
            "If omitted, OctoAgent will infer the default provider class from interface_type or provider_name."
        ),
    )
    ...
    interface_type: str | None = Field(
        default=None,
        description=(
            "Normalized provider interface/dialect. Preferred over vendor-specific `use` when using built-in integrations "
            "(for example: openai_compatible, anthropic_messages, google_genai, deepseek_reasoner)."
        ),
    )
    provider_name: str | None = Field(
        default=None,
        description=(
            "Optional provider/vendor label used for documentation and interface inference "
            "(for example: openai, openrouter, groq, ollama, anthropic, google, deepseek)."
        ),
    )
    ...
    def resolved_use(self) -> str:
        ...
        raise ValueError(
            f"Model {self.name} must define `use` or a known `interface_type`/`provider_name` so OctoAgent can resolve the provider class."
        ) from None
