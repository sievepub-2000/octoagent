...
class SystemGuardConfig(BaseModel):
    ...
    runtime_embeddings_enabled: bool = Field(
        default=False,
        description=(
            "Whether system guard should call embedded runtime model for vector embeddings. "
            "Disabled by default to keep startup/shutdown snapshots robust in environments "
            "where llama-cpp runtime may be unstable."
        ),
    )
