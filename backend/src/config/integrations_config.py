...
class WebhookIngressConfig(BaseModel):
    ...
    auth_methods: list[IngressAuthMethod] = Field(
        default_factory=lambda: ["hmac_signature", "bearer_token", "api_key"]
    )
...
class ApiIngressConfig(BaseModel):
    enabled: bool = Field(default=False)
    auth_methods: list[IngressAuthMethod] = Field(
        default_factory=lambda: ["bearer_token", "api_key", "oauth2_client_credentials"]
    )
...
class BrowserAutomationConfig(BaseModel):
    enabled: bool = Field(default=False)
    engine: Literal["playwright", "remote_cdp", "none"] = Field(default="playwright")
    ...
    note: str = Field(
        default=(
            "Prefer a standard browser automation engine with reusable session controls. "
            "Do not assume a bespoke embedded browser runtime unless explicitly configured."
        )
    )
...
class SystemExecutionPermissionPolicyConfig(BaseModel):
    ...
    rules: list[SystemExecutionPermissionRuleConfig] = Field(
        default_factory=_default_system_execution_rules
    )
...
class SystemExecutionConfig(BaseModel):
    ...
    permission_policy: SystemExecutionPermissionPolicyConfig = Field(
        default_factory=SystemExecutionPermissionPolicyConfig
    )
    note: str = Field(
        default=(
            "System-level operation is available through sandbox execution and system-execution planning APIs. "
            "Desktop-native control remains a scaffold and still requires an explicit desktop provider."
        )
    )
