...
class SystemGuardVectorStore:
    ...
    def close_running_sessions(self, *, reason: str, updated_at: str) -> int:
        with self._connect() as conn:
            running = conn.execute(
                "SELECT session_id, state_json FROM system_guard_sessions WHERE status = 'running'"
            ).fetchall()
            for row in running:
    ...
    def list_running_sessions(self) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT session_id, updated_at, state_json FROM system_guard_sessions WHERE status = 'running'"
            ).fetchall()
        return [
            {
                "session_id": row[0],
    ...
    def count_snapshots(self, *, namespace: str | None = None) -> int:
        with self._connect() as conn:
            if namespace is None:
                row = conn.execute(
                    "SELECT COUNT(*) FROM system_guard_vectors"
                ).fetchone()
            else:
                row = conn.execute(
