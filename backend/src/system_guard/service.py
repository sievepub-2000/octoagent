...
from src.config.paths import get_paths, resolve_configured_default_model_name
from src.config.system_guard_config import get_system_guard_config
...
class SystemGuardService:
    ...
    def _config_overview(self) -> dict[str, Any]:
        ...
        return {
            "default_model": resolve_configured_default_model_name(
    ...
    def _call_default_agent_repair(self, issues: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def _invoke() -> str:
            ...
            from src.client import OctoAgentClient
    ...
    def _should_invoke_default_agent(self, issues: list[dict[str, Any]]) -> bool:
        if not issues or not self._config.invoke_default_agent_on_issue:
            return False
        return any(
            issue.get("severity") == "critical" or issue.get("auto_repairable") is True
            for issue in issues
        )
    ...
    def _embedding_for_state(self, state: dict[str, Any]) -> tuple[str, list[float]]:
        content = (
            ...
            f"default_model={(state.get('config_overview') or {}).get('default_model')} "
            f"issues={json.dumps(state.get('issues', []), ensure_ascii=False)}"
        )
    ...
    def run_manual_repair(self, *, advisory_only: bool = False) -> dict[str, Any]:
        ...
        stale_running = [
            session
            for session in self._store.list_running_sessions()
