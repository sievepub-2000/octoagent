...
from src.subagents.executor import cleanup_background_task, get_background_task_result
from src.subagents.policy import resolve_subagent_config
...
def _extract_search_topic_terms(query: str) -> list[str]:
    ...
    ascii_tokens = [
        ...
        if token not in _ASCII_QUERY_STOPWORDS and not token.isdigit() and token != "x.com"
    ]
    ...
    if _looks_like_news_query(normalized):
        if re.search(r"\b(top|trending|headline|headlines)\b", lowered) or any(
            marker in normalized for marker in ("前十", "头条", "热门", "热搜", "十大")
        ):
            topic_terms.append("trending")
        if re.search(r"\b(today|latest|recent)\b", lowered) or any(
            marker in normalized for marker in ("今天", "今日", "最新")
        ):
...
def _run_shell(runtime: ToolRuntime[ContextT, ThreadState], command: str) -> str:
...
def glob_tool(runtime: ToolRuntime[ContextT, ThreadState], pattern: str, root: str = "/mnt/user-data/workspace") -> str:
    ...
    return _run_shell(runtime, cmd)


@tool("grep", parse_docstring=True)
def grep_tool(runtime: ToolRuntime[ContextT, ThreadState], query: str, root: str = "/mnt/user-data/workspace") -> str:
...
def web_search_tool(query: str, max_results: int = 5) -> str:
    ...
    for candidate_query in queries:
        ...
        try:
            ...
            parsed = [
                {"title": _clean_search_text(title_html), "href": href, "snippet": "", "published": ""}
                for href, title_html in links
                if href and _clean_search_text(title_html)
            ]
            if parsed:
                _record_results(parsed, candidate_query)
                if len(ranked_results) >= max_results:
    ...
    if ranked_results:
        ...
        if strict_domain_match and required_domain:
            ...
            if not ranked:
                return (
                    f"Search results unavailable: required domain {required_domain} not found for strict query. "
                    f"Query: {query}"
                )
    ...
    if fallback_pool:
        ...
        if strict_domain_match and required_domain:
            ...
            deduped_pool = [
                item for item in deduped_pool if _search_domain(item.get("href", "")) == required_domain
            ]
    ...
    header = (
        "Search backend unavailable right now; returning fallback public sources "
        f"for manual verification. Tried backends: {', '.join(errors)}"
    )
...
def lsp_tool(runtime: ToolRuntime[ContextT, ThreadState], symbol: str, root: str = "/mnt/user-data/workspace") -> str:
...
def mcp_auth_tool(server: str) -> str:
    ...
    """
    return (
        "MCP auth is configured via extensions_config.json env/oauth settings. "
        f"Server='{server}'. Please configure token/env and reinitialize MCP tools."
    )
...
def read_mcp_resource_tool(resource: str) -> str:
    ...
    """
    return (
        "Direct MCP resource read is transport-specific in this build. "
        f"Use MCP tool calls directly or inspect available tools. Requested: {resource}"
    )
