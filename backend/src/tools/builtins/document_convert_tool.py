...
from src.tools.sandbox.tools import get_thread_data, replace_virtual_path
...
<body>{html}</body>
    ...
    if suffix == ".csv" and target in ("markdown", "md"):
        ...
        with open(path, newline="", encoding="utf-8", errors="replace") as f:
            ...
            rows = list(reader)
        if not rows:
            return Command(update={"messages": [ToolMessage("CSV file is empty.", tool_call_id=tool_call_id)]})
        header = rows[0]
        lines = ["| " + " | ".join(header) + " |"]
    ...
    return Command(
        update={
            "messages": [
                ToolMessage(
                    f"Error: Unsupported conversion {suffix} → {target}. Supported: Office→markdown, HTML→markdown, Markdown→html, CSV→markdown, JSON→markdown, any→text.",
                    tool_call_id=tool_call_id,
                )
            ]
        }
