...
class SystemExecutionRuntimeExecutor:
    ...
    def _execute_command_step(
        ...
        command = session.requested_commands[-1] if next_command_index >= len(session.requested_commands) else session.requested_commands[next_command_index]
        ...
        if remaining_commands > 0:
            detail = f"Executed command in {working_directory}: {command}\n{command_output}\n{remaining_commands} requested command(s) remain in this act step."
