...
def build_servers_config(extensions_config: ExtensionsConfig) -> dict[str, dict[str, Any]]:
    ...
    for server_name, server_config in enabled_servers.items():
        try:
            missing_env = [key for key, value in server_config.env.items() if not str(value or "").strip()]
            if missing_env:
                logger.warning(
                    "Skipping MCP server '%s' because required environment variables are unresolved: %s",
                    server_name,
                    ", ".join(missing_env),
                )
                continue
