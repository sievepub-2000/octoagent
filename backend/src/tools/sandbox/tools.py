...
from src.tools.sandbox.exceptions import (
    SandboxError,
...
def get_runtime_thread_id(runtime: ToolRuntime[ContextT, ThreadState] | None) -> str | None:
    ...
    for layer_name in ("configurable", "metadata"):
        layer = config.get(layer_name)
        if isinstance(layer, dict):
            thread_id = layer.get("thread_id") or layer.get("threadId")
            if thread_id:
    ...
    if isinstance(runtime.state, dict):
        ...
        if isinstance(messages, list) and messages:
            ...
            if message_id:
                thread_id = f"message-{message_id}"
                context["thread_id"] = thread_id
                return thread_id
            content = str(getattr(messages[0], "content", "") or "")
            if content:
                digest = hashlib.sha1(content.encode("utf-8")).hexdigest()[:16]
                thread_id = f"message-{digest}"
                context["thread_id"] = thread_id
                return thread_id
...
def _build_thread_data(thread_id: str) -> ThreadDataState:
    paths = get_paths()
    return {
...
def get_thread_data(runtime: ToolRuntime[ContextT, ThreadState] | None) -> ThreadDataState | None:
    ...
    thread_data = _build_thread_data(thread_id)
...
async def bash_tool(runtime: ToolRuntime[ContextT, ThreadState], description: str, command: str) -> str:
    ...
    try:
        ...
        # NOTE: ``LocalSandbox.execute_command`` is async; this tool MUST be async
        # and await the coroutine. Returning the coroutine directly (the previous
        # sync form) produced the smoking-gun warning
        # ``RuntimeWarning: coroutine 'LocalSandbox.execute_command' was never
        # awaited`` and shipped a ``<coroutine object ...>`` string back to the
        # model, which then retried forever until LangGraph's recursion ceiling.
