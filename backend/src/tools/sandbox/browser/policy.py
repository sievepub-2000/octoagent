...
class BrowserSessionPolicy:
    ...
    def remaining_action_ids(self, session: BrowserExecutionSession) -> list[str]:
        return [action.action_id for action in session.planned_actions if action.action_id not in session.executed_action_ids]
    ...
    def refresh_pending_state(self, session: BrowserExecutionSession) -> None:
        ...
        session.recovery_available = session.status == "failed" and bool(session.current_url and session.pending_action_ids)
