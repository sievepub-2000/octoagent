...
class ExecutionNodeRegistry:
    ...
    def route_task(self, task_id: str, *, affinity_node: str | None = None) -> TaskRoutingDecision:
        ...
        # Prefer independent workers over the gateway-local fallback, then pick
        # the least-loaded node in that class.
        best = min(candidates, key=lambda n: (n.node_id == "local", n.current_load))
        return TaskRoutingDecision(
            task_id=task_id,
    ...
    async def _post_worker_dispatch(
        self,
        node: ExecutionNode,
        *,
        dispatch_id: str,
    ...
    async def dispatch_task(
        ...
        *,
        ...
        candidates.extend(node for node in sorted(self.healthy_nodes(), key=lambda item: item.current_load) if node.node_id not in {candidate.node_id for candidate in candidates} and node.available_capacity > 0)
        if not candidates:
        ...
        for node in candidates:
            ...
            try:
                if node.node_id == "local":
                    ...
                    result_payload = await self._post_worker_dispatch(
                        node,
                        dispatch_id=dispatch_id,
                        lease_id=lease_id,
                        task_id=task_id,
