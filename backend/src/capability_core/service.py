...
from src.skills.loader import get_skills_root_path, invalidate_skills_cache
...
def _source_root() -> Path:
    return Path(os.getenv("OCTO_AGENT_CAPABILITY_SOURCE", "/home/sieve-pub/Vscode-workspace")).expanduser().resolve()
...
class CapabilityCoreService:
    ...
    def build_inventory(self) -> dict[str, object]:
        ...
        matched = {
            category: sorted(set(source[category]).intersection(installed[category]))
    ...
    def build_runtime_state(self) -> dict[str, object]:
        ...
        compat_root = (
            resolve_agent_skills_source_root(compat_config, allow_disabled=True)
    ...
    def build_registry_snapshot(self) -> dict[str, object]:
        ...
        compat_index = {
            ...
            for item in compat_preview.get("items", [])
        }
