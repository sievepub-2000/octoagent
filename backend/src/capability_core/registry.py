...
from src.capability_core.agent_skills_compat import (
    ...
    for plugin_id in sorted(set(capabilities) | set(manifests) | set(registry_entries)):
        ...
        items.append(
            UnifiedCapabilityItem(
                ...
                version=(
                    registry_entry.installed_version
                ...
                metadata={
                    "execution_mode": (
                        capability.execution_mode if capability is not None else None
                    ),
                    "category": capability.category if capability is not None else None,
                    "installation_targets": (
                        manifest.installation_targets if manifest is not None else []
                    ),
...
def _channel_items() -> list[UnifiedCapabilityItem]:
    ...
    for name, raw_channel in sorted(channels.items()):
        ...
        required_fields = [
            str(field.get("name"))
            for field in fields
            if field.get("required") and str(field.get("name") or "").strip()
        ]
...
def _hook_items(
    ...
    hook_names = sorted(
        source_hooks
        | installed_hooks
        | set(binding_events.keys())
        | set(webhook_events.keys())
    )
    ...
    for hook_name in hook_names:
        ...
        if repo_known:
            ...
            enabled = binding_enabled.get(hook_name, False) or webhook_enabled.get(
                hook_name, False
            )
        provides = _dedupe_preserve_order(
            binding_events.get(hook_name, []) + webhook_events.get(hook_name, [])
        )
        source = "repo"
