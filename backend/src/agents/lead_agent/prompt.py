...
**网页信息获取**:
1. web_search → 获取搜索结果和摘要（优先用于发现信息）
2. web_fetch → 获取完整网页内容（用于深入阅读已知URL）
3. read_webpage → 提取文章正文内容（用于结构化提取）
4. scrapling_fetch → 反爬虫页面的最后手段
...
3) **立即重新执行**：不要把失败当作任务终止的理由，除非已尝试3种以上不同方案
