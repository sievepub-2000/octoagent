...
def build_workspace_session_updates(workspace: Any, latest_session: QuerySession) -> dict[str, object]:
    updates: dict[str, object] = {
        ...
        "active_query_session_count": sum(1 for agent in workspace.agents if agent.metadata.get("query_session_id")),
    }
...
def sync_workspace_session_state(workspace: Any, *, query_service) -> bool:
    ...
    for agent in workspace.agents:
        session = query_service.latest_session_for_agent(workspace.task_id, agent.agent_id)
        if session is None:
            continue
    ...
    if latest_session is not None:
        ...
        if any(workspace.metadata.get(key) != value for key, value in workspace_updates.items()):
...
def build_handoff_summary(
    ...
    management_agent = next(
        (agent for agent in agents if agent.role in {"lead", "coordinator", "manager", "orchestrator"}),
    ...
    for session in sorted(query_sessions, key=lambda s: s.created_at, reverse=True):
        if management_agent is None or session.agent_id == management_agent.agent_id:
            continue
        target_agent = next((a for a in agents if a.agent_id == session.agent_id), None)
        target_metadata = target_agent.metadata if target_agent is not None and isinstance(target_agent.metadata, dict) else {}
        latest_turn = session.turns[-1] if session.turns else None
        handoffs.append(
            {
                ...
                "linked_card_id": (target_agent.linked_card_id if target_agent is not None else None),
                "created_at": session.created_at,
                "summary": (latest_turn.assistant_summary if latest_turn is not None else None),
            }
        )
    return handoffs
