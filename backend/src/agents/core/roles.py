...
def select_reviewer_agent(agents: list[Any], *, exclude_agent_id: str | None = None):
    return next(
        (agent for agent in agents if is_reviewer_role(getattr(agent, "role", None)) and (exclude_agent_id is None or getattr(agent, "agent_id", None) != exclude_agent_id)),
        None,
    )
...
def split_execution_roles(workspace) -> tuple[Any, list[Any], Any | None]:
    ...
    worker_agents = [agent for agent in agents if getattr(agent, "agent_id", None) != getattr(lead_agent, "agent_id", None) and (review_agent is None or getattr(agent, "agent_id", None) != getattr(review_agent, "agent_id", None))]
