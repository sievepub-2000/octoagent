...
class MemoryMiddleware(AgentMiddleware[MemoryMiddlewareState]):
    ...
    def after_agent(self, state: MemoryMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        # Falls back to raw excerpt storage if the bridge is unavailable.
        try:
            ...
            # Graceful fallback: store raw excerpt (original behaviour)
            try:
                summary_parts: list[str] = []
                for m in filtered_messages[-6:]:  # last 3 exchanges max
                ...
                if summary_parts:
                    ...
                    store.add(
                        ...
                        metadata={
                            "thread_id": thread_id,
                            "confidence": max(config.fact_confidence_threshold, 0.75),
                            "source": "memory_middleware_fallback",
                            "source_kind": "fallback_summary",
                            "pipeline": "memory_middleware",
                        },
                    )
            except Exception:
                pass  # Never block the main flow for system memory
