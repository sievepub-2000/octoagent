"""Stage execution review middleware for long-running agent work.

The middleware turns runtime signals into soft self-review checkpoints.  It
does not stop the agent for ordinary tool errors; instead it asks the model to
audit progress, record the next safe action, and continue with a changed
...
_REVIEW_MARKER = '<execution_review origin="execution_review_middleware"'
_REVIEW_INTERVAL_SECONDS = int(os.getenv("OCTO_EXECUTION_REVIEW_INTERVAL_SECONDS", "300"))
_MAX_ERROR_LINES = 6


class ExecutionReviewMiddlewareState(AgentState):
    """State used by the execution review middleware."""
...
def _active_task_state(state: ExecutionReviewMiddlewareState) -> dict[str, Any] | None:
...
def _elapsed_review_due(runtime_state: dict[str, Any], state: ExecutionReviewMiddlewareState) -> bool:
...
class ExecutionReviewMiddleware(AgentMiddleware[ExecutionReviewMiddlewareState]):
    """Inject soft execution-review checkpoints before model calls."""

    state_schema = ExecutionReviewMiddlewareState

    @override
    def before_model(
        self,
        state: ExecutionReviewMiddlewareState,
...
__all__ = ["ExecutionReviewMiddleware"]
