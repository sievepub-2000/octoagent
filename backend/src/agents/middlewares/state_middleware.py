"""Merged state middleware: thread data setup + persistent task state."""
...
def _dedupe_progress(completed_steps: list[Any], pending_steps: list[Any]) -> tuple[list[Any], list[Any]]:
    completed: list[Any] = []
...
def _runtime_update_for_task(task_state: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime_update: dict[str, Any] = {"task_state_status": status, "updated_at": _utc_now()}
...
class StateMiddleware(AgentMiddleware[StateMiddlewareState]):
    """Set up thread data directories and manage persistent task state."""
    ...
    @override
    def after_agent(self, state: StateMiddlewareState, runtime: Runtime) -> dict[str, Any] | None:
        messages = list(state.get("messages") or [])
    ...
    @override
    async def abefore_agent(self, state: StateMiddlewareState, runtime: Runtime) -> dict[str, Any] | None:
        return self.before_agent(state, runtime)

    @override
    async def aafter_agent(self, state: StateMiddlewareState, runtime: Runtime) -> dict[str, Any] | None:
