...
- For agent self-operation, preserve explicit user preferences about long-running work,
  soft budgets, tool-use review points, retries, and when NOT to enforce fixed hard caps.
  If the conversation establishes an operational soft policy, keep the wording specific
  enough that future system memory can recover values such as "tool soft budget 80".
- Confidence levels:
...
def format_memory_for_injection(memory_data: dict[str, Any], max_tokens: int = 2000) -> str:
    ...
    if isinstance(memory_blocks, dict) and memory_blocks:
        block_lines = ["<memory_blocks>"]
        for label, raw in sorted(memory_blocks.items()):
            if not isinstance(raw, dict):
                continue
            value = str(raw.get("value") or "")
            if not value.strip():
                continue
            description = str(raw.get("description") or f"Memory block '{label}'.")
            limit = int(raw.get("limit") or 5000)
            ...
            block_lines.extend(
                [
                    ...
                    "</metadata>",
                    f"<value>{value[:limit]}</value>",
                    f"</{label}>",
                ]
            )
        block_lines.append("</memory_blocks>")
    ...
    if history_data:
        ...
        if long_term.get("summary"):
            history_sections.append(f"Long-term: {long_term['summary']}")
