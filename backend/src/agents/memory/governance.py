...
def resolve_memory_expiry(metadata: dict[str, Any]) -> datetime | None:
    ...
    expires_at = _parse_datetime(str(metadata.get("expires_at") or metadata.get("expires_on") or ""))
...
def _build_retention_policy(
    ...
    explicit_expires = _parse_datetime(str(metadata.get("expires_at") or metadata.get("expires_on") or ""))
...
def build_memory_governance_summary(config: MemoryConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "immutable_namespaces": [namespace for namespace in PERMANENT_MEMORY_NAMESPACES if effective_config.permanent_memory_immutable],
