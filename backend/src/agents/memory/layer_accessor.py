...
class MemoryLayerAccessor:
    ...
    def store_long_term_memory(
        ...
        *,
        ...
        return (
            self.governed_store_long_term_memory(
                unit,
                namespace=namespace,
        ...
        *,
        ...
        return LayeredMemoryContext(
            ...
            long_term=MemoryLayerSnapshot(
                ...
                governance={namespace: governance_summary["namespace_policies"][namespace] for namespace in LONG_TERM_MEMORY_NAMESPACES},
            ),
            permanent=MemoryLayerSnapshot(
                layer="permanent",
                namespaces=PERMANENT_MEMORY_NAMESPACES,
                entries=permanent_entries,
                governance={namespace: governance_summary["namespace_policies"][namespace] for namespace in PERMANENT_MEMORY_NAMESPACES},
            ),
        )
            ...
            for entry in store.search(query, namespace=namespace, top_k=top_k):
                results.append(
                    MemorySearchEntry(
                        ...
                        confidence=float(entry.metadata.get("memory_confidence")) if entry.metadata.get("memory_confidence") is not None else None,
