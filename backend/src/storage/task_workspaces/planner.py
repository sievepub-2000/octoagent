...
class TaskWorkspacePlanner:
        ...
        context = BrainTaskContext(
            ...
            memory_hints=[
                ...
                for item in [
                    ...
                    ("checkpoint_labels=" + ", ".join(checkpoint.label for checkpoint in workspace.checkpoints[:3]) if workspace.checkpoints else ""),
                ]
                if item
            ],
        )
        ...
        workspace.metadata.update(
            {
                ...
                "compiled_ready_card_count": sum(1 for card in compiled_graph.cards if card.runtime_binding is not None and card.runtime_binding.state == "ready"),
                "compiled_blocked_card_count": sum(1 for card in compiled_graph.cards if card.runtime_binding is not None and card.runtime_binding.state == "blocked"),
                "compiled_review_card_count": sum(1 for card in compiled_graph.cards if card.kind == "review"),
                "compiled_checkpoint_card_count": sum(1 for card in compiled_graph.cards if card.kind == "checkpoint"),
                "compiled_requires_final_review": any(handoff.destination == "review_queue" for handoff in compiled_graph.handoffs),
                "research_experiment_id": research_experiment_id,
    ...
    def _brain_constraints(self, workspace: TaskWorkspace, builder_draft: dict) -> list[str]:
        constraints = [item for item in workspace.metadata.get("constraints", []) if isinstance(item, str) and item.strip()]
    ...
    def _compiled_graph_to_task_graph(
        ...
        *,
        ...
        existing_project = next(
            (card for card in existing_cards if card.kind == "start" or "project" in card.tags or "entry" in card.tags),
            None,
        )
        existing_by_agent = {card.linked_agent_id: card for card in existing_cards if card.linked_agent_id is not None}
        ...
        for index, agent in enumerate(workspace.agents):
            ...
            card_id = existing_card.card_id if existing_card is not None else agent.linked_card_id or make_id("card")
    ...
    def _prompt_preview(self, workspace: TaskWorkspace, agent, *, is_primary: bool) -> str:
        if is_primary:
            if workspace.mode == "branch":
                return f"你是主协调 agent。任务目标：{workspace.goal or workspace.name}。\n先拆分分支任务，再监督各子 agent 返回可验证结果，最后汇总。"
            if workspace.mode == "group":
                return f"你是群聊主 agent。任务目标：{workspace.goal or workspace.name}。\n负责维持群组协作节奏，并基于各成员真实产出做最终收口。"
            return f"你是主执行 agent。任务目标：{workspace.goal or workspace.name}。\n直接完成任务，输出可验证结果与必要证据。"
        return f"你是 {agent.name}，职责：{agent.task_scope or agent.role}。\n围绕总任务“{workspace.goal or workspace.name}”只完成你负责的分支工作，返回可验证结果。"
    ...
    def _resolve_linked_agent(self, workspace: TaskWorkspace, card) -> tuple[object | None, int]:
        ...
        if card.kind == "research":
            fallback = next(
                (agent for agent in workspace.agents if any(token in {"research", "researcher"} for token in self._agent_tokens(agent))),
                None,
            )
