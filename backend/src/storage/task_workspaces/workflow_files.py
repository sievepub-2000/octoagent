...
class WorkflowFileManager:
    ...
    def update_workflow_file(
        ...
        *,
        ...
        card_by_agent_id = {str(card.get("linked_agent_id")): card for card in cards if isinstance(card, dict) and card.get("linked_agent_id")}
        lines = [
            "# Workflow Settings",
            "",
            f"- Task ID: `{task_id}`",
    ...
    def read_result_payload(self, task_id: str) -> dict[str, Any]:
        ...
        available_sources = [name for name in (CANONICAL_RESULT_DOC, LEGACY_RESULT_DOC, RUN_LOG_DOC) if (archive_dir / name).exists()]
    ...
    def read_agent_conversation(self, task_id: str, agent_id: str) -> list[dict[str, Any]] | None:
        ...
        try:
            ...
            logger.warning("Failed to read agent conversation archive for task=%s agent=%s", task_id, agent_id, exc_info=True)
            record_exception_trace("task_workspaces.workflow_files.read_agent_conversation", exc, task_id=task_id, agent_id=agent_id)
            return None
    ...
    def read_workflow_state(self, task_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.warning("Failed to read workflow state archive for task=%s", task_id, exc_info=True)
            record_exception_trace("task_workspaces.workflow_files.read_workflow_state", exc, task_id=task_id)
            return None
    ...
    def update_result_file(
        ...
        *,
        ...
        if artifacts:
            ...
            result_lines.extend([f"- [{artifact.get('path', artifact.get('name', 'artifact'))}]({artifact.get('path', artifact.get('name', 'artifact'))})" for artifact in artifacts])
            result_lines.append("")
        result_lines.extend(
            [
                "## Transcripts",
    ...
    def _write_final_results_section(
        ...
        *,
        ...
        if artifacts:
            ...
            final_lines.extend([f"- [{artifact.get('path', artifact.get('name', 'artifact'))}]({artifact.get('path', artifact.get('name', 'artifact'))})" for artifact in artifacts])
            final_lines.append("")
    ...
    def _read_index_from(self, state_dir: Path, index_path: Path) -> dict[str, str]:
        ...
        try:
            ...
            logger.warning("Failed to read workflow archive index at %s", index_path, exc_info=True)
            record_exception_trace("task_workspaces.workflow_files.read_index", exc, index_path=str(index_path))
            return {}
