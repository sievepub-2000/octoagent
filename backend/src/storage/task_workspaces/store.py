...
class TaskWorkspaceStore:
    ...
    def list_workspaces(self) -> list[TaskWorkspace]:
        with self._lock:
            payload = self._read()
            return [TaskWorkspace.model_validate(workspace) for workspace in payload.get("workspaces", [])]
    ...
    def list_agent_messages(self, task_id: str, agent_id: str) -> list[AgentMessage]:
        with self._lock:
            ...
            if task_local_messages is not None:
                return [AgentMessage.model_validate(message) for message in task_local_messages]
            payload = self._read()
            key = f"{task_id}:{agent_id}"
            return [AgentMessage.model_validate(message) for message in payload.get("agent_messages", {}).get(key, [])]
    ...
    def delete_workspace(self, task_id: str) -> bool:
        with self._lock:
            ...
            remaining_workspaces = [workspace for workspace in workspaces if workspace.get("task_id") != task_id]
            agent_messages = payload.get("agent_messages", {})
            remaining_messages = {key: value for key, value in agent_messages.items() if not key.startswith(f"{task_id}:")}
            changed = len(remaining_workspaces) != len(workspaces) or len(remaining_messages) != len(agent_messages)
            if not changed:
                return False
