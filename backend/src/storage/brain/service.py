...
class BrainCoreService:
    ...
    def run(self, context: BrainTaskContext) -> BrainResponse:
        ...
        if strategy_validation.warnings:
            analysis.findings.extend([f"Fusion warning: {warning}" for warning in strategy_validation.warnings])
        if strategy_validation.errors:
            analysis.risks.extend([f"Fusion error: {error}" for error in strategy_validation.errors])
    ...
    async def execute_plan(
        ...
        *,
        ...
        execution_summary: dict[str, Any] = {
            ...
            "strategy_valid": (not brain_response.strategy_validation.errors if brain_response.strategy_validation else True),
        }
