...
class BrainExecutionContractBuilder:
    ...
    def _memory_contract_note(
        ...
        *,
        ...
        return f"Memory context: strength={memory_context_strength}, hint_count={hint_count}."
    ...
    def _build_quant_contract(
        ...
        checkpoints = [
            ...
            BrainApprovalCheckpoint(
                ...
                status=("pending" if decision.recommendation.startswith("Proceed with bounded quant exploration") else "blocked"),
                phase="approval",
                reason=(None if decision.recommendation.startswith("Proceed with bounded quant exploration") else "Execution path is not ready for operator approval yet."),
        ...
        readiness, execution_phase, next_action, approval_handoff = self._resolve_quant_execution_state(
        ...
        return BrainExecutionContract(
            ...
            quant_backtest=BrainQuantBacktestContract(
                ...
                suggested_universe=("constrained" if context.constraints or context.risk_limits else "broad_market"),
                execution_phase=execution_phase,
                next_action=next_action,
                approval_handoff=approval_handoff,
            ),
    ...
    def _format_quant_execution_state_note(
        ...
        *,
        ...
        return f"Execution state: phase={execution_phase}, next_action={next_action}, approval_handoff={approval_handoff}."
    ...
    def _format_contract_summary_note(
        ...
        *,
        ...
        return f"Contract state: readiness={readiness}, current_phase={current_phase}, next_owner={next_owner}."
    ...
    def _build_generic_contract(
        ...
        review_needed = decision.recommendation.startswith("Require manual review")
        readiness: Readiness = "review_required" if review_needed else "ready"
        current_phase: CurrentPhase = "plan"
        next_owner: NextOwner = "operator" if review_needed else "system"
        memory_context_strength = self._memory_context_strength(context)
        review_intensity = self._review_intensity(
            decision=decision,
            missing_inputs=[],
