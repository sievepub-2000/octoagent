...
class BrainMemoryReasoner:
    ...
    def analyze(self, context: BrainTaskContext) -> BrainAnalysis:
        ...
        if context.memory_hints:
            findings.append("Memory hints available for plan shaping: " + ", ".join(context.memory_hints[:5]))
            confidence += 0.2
        else:
        ...
        if context.preferred_mode in {"research", "policy"} and not context.memory_hints:
            risks.append("Cross-session memory hints are absent for a mode that benefits from prior context.")
