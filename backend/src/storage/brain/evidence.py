...
class BrainEvidenceRouter:
    ...
    def analyze(self, context: BrainTaskContext) -> BrainAnalysis:
        ...
        if context.evidence:
            findings.append(f"Evidence router accepted {len(context.evidence)} evidence item(s) for downstream planning.")
            confidence += 0.2
        else:
            risks.append("Evidence router could not validate execution readiness without evidence.")

        if context.constraints:
            findings.append(f"Constraint snapshot available for routing: {len(context.constraints)} item(s).")
            confidence += 0.05
