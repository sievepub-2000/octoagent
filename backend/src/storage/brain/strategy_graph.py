    ...
    def validate(self, graph: StrategyGraph) -> StrategyGraphValidationReport:
        ...
        for edge in graph.edges:
            ...
            if edge.kind == "feedback_lagged":
                if edge.lag < 1:
                    errors.append(f"Lagged feedback edge must have lag >= 1: {edge.source} -> {edge.target}")
            else:
                if edge.lag != 0:
                    warnings.append(f"Non-feedback edge should have lag 0: {edge.source} -> {edge.target}")
        ...
        for output_name, owners in producers.items():
            ...
            if arbitration is None:
                errors.append(f"Output '{output_name}' is produced by multiple strategies {owners} without arbitration.")
                continue
            missing = sorted(set(owners) - set(arbitration.owners))
            if missing:
                errors.append(f"Arbitration for '{output_name}' does not cover all owners: missing {missing}.")
            if arbitration.mode == "single_owner":
                errors.append(f"Output '{output_name}' has multiple producers but arbitration mode is single_owner.")
            if arbitration.mode == "weighted_vote":
                warnings.append(f"Output '{output_name}' uses weighted_vote. Prefer stacked_meta or policy_gate for high-risk decisions.")
