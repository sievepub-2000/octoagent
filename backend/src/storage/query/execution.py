...
class QueryTurnExecutor:
    ...
    def extract_signal_message(self, message: str) -> str:
        ...
        for marker in _TASK_GOAL_PROMPT_MARKERS:
            ...
            lowered = tail.lower()
            cut_positions = [lowered.find(section_marker) for section_marker in _TASK_PROMPT_SECTION_MARKERS if lowered.find(section_marker) >= 0]
            if cut_positions:
                tail = tail[: min(cut_positions)]
            return tail.strip()
        return candidate

    def is_conversation_request(self, message: str) -> bool:
        signal_message = self.extract_signal_message(message).strip().lower()
    ...
    def execute_system_target(self, session, message: str, *, allow_side_effects: bool, client_command: QueryClientCommand | None = None):
        ...
        if cli_scope == "system":
            if permission_mode != "system":
                return "system_execution", "System-level CLI requires system permission mode.", 0, "blocked", None, None
            target = "system_cli"
    ...
    def execute_read_target(self, session, message: str, client_command: QueryClientCommand | None = None):
        ...
        detail = f"Read-oriented turn executed against repository context. Current guidance: {analysis} Primary goal: {session.current_goal}. Request: {message}"
