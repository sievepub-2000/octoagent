...
from src.agents.core.roles import has_reviewer_agent
from src.runtime.config.ml_intern_defaults import ML_INTERN_HF_MCP_SERVER, build_ml_intern_runtime_context, resolve_ml_intern_profile_name
...
class QuerySessionProfileAssembler:
    ...
    def update_memory_profile(self, session: QuerySession) -> None:
        ...
        if scope_weights:
            ...
            if len(ranked_scopes) == 1 or (len(ranked_scopes) > 1 and abs(ranked_scopes[0][1] - ranked_scopes[1][1]) >= 0.1):
    ...
    def build_tool_registry(
        ...
        *,
        ...
        workspace_approval = permission_mode == "approval"
        system_approval = permission_mode != "system"
        browser_approval = permission_mode != "system"
        tools = [
            QueryToolDescriptor(
                tool_id="repo-read",
    ...
    def build_task_analysis(
        ...
        *,
        ...
        coordination_strategy = str(workspace.metadata.get("coordination_strategy") or ("coordinator_workers" if workspace.mode == "branch" else ("manager_review" if workspace.mode == "group" else "solo")))
        flow = [
        ...
        if permission_mode == "approval":
            self_review_checklist.append("Ask for explicit approval before directory or system actions.")
        elif permission_mode == "directory":
            self_review_checklist.append("Stay scoped to directory-level operations unless the operator raises the permission mode.")
        ...
        return QueryTaskAnalysis(
            ...
            summary=(f"Agent '{agent.name}' is preparing to work on '{workspace.name}' using a repository-owned prompt stack, shared tool registry, and bounded handoff workflow."),
    ...
    def render_module_content(
        ...
        *,
        ...
        tool_lines = [f"{tool.title} [{tool.kind}] approval={'yes' if tool.requires_approval else 'no'}" for tool in tool_registry if tool.enabled]
        mcp_lines = [f"{server.server_id} via {server.transport} auth={server.auth_mode}" for server in mcp_servers if server.enabled]
