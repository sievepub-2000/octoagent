...
class BrainBuilderActionModelBuilder:
    ...
    def build(
        ...
        if current_workflow_mode != execution_contract.suggested_workflow_mode:
            auto_actions.append(
                BrainBuilderAction(
                    ...
                    description=(
                        f"Switch workflow mode from {current_workflow_mode} to "
                        f"{execution_contract.suggested_workflow_mode} to match the Brain contract."
                    ),
                    auto_apply=True,
        ...
        if context.preferred_mode != suggested_brain_mode:
            auto_actions.append(
                BrainBuilderAction(
                    ...
                    description=(
                        f"Switch preferred Brain mode to {suggested_brain_mode} based on the "
                        f"{execution_contract.template} contract template."
                    ),
                    auto_apply=True,
        ...
        if (
            current_on_final_failure != suggested_on_final_failure
            or current_max_total_steps != str(suggested_max_total_steps)
        ):
            auto_actions.append(
                BrainBuilderAction(
        ...
        if execution_contract.checkpoints:
            pending_required = [
                checkpoint.title
                for checkpoint in execution_contract.checkpoints
                if checkpoint.required and checkpoint.status != "ready"
            ]
            if pending_required:
                manual_actions.append(
                    BrainBuilderAction(
                        ...
                        description=(
                            "The workflow still has unresolved approval gates: "
        ...
        summary = (
            f"Brain generated {len(auto_actions)} auto-applicable builder actions and "
            f"{len(manual_actions)} manual follow-ups for the {execution_contract.template} contract."
        )
    ...
    def _mode_specific_patch(
        ...
        return {
            ...
            "collaborationStyle": (
                "deep_review"
                if execution_contract.review_intensity == "heightened"
                else "balanced"
            ),
        }
