...
class OrchestrationCompiler:
    ...
    def get_seed_graph(self) -> CompiledTaskGraph:
        return CompiledTaskGraph(
            ...
            task_id=None,
            source_plan_summary=("Compile a Brain plan into a task graph with agent, browser, research, and review cards."),
            cards=[
                self._cards.create(
            ...
            cards.append(
                self._cards.create(
                    ...
                    runtime_binding=RuntimeBinding(
                        ...
                        notes=[
                            step.description,
                            (f"Dependency gate: waits for '{previous_card_id}' before execution." if dependency_ids else "Dependency gate: none."),
                        ],
                    ),
                ...
                self._cards.create(
                    ...
                    runtime_binding=RuntimeBinding(
                        ...
                        notes=[
                            ...
                            (f"Next step: {checkpoint.next_step}" if checkpoint.next_step is not None else "Next step: not specified."),
                        ],
                    ),
