            ...
            PromptStackProfile(
                ...
                modules=[
                    ...
                    PromptModuleProfile(
                        ...
                        instruction_template=(
                            "Understand the goal, inspect the relevant code and docs, choose the smallest valid execution path, implement only what is needed, verify with tests or direct checks, and report concrete outcomes."
                        ),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Treat the repository context snapshot as the working baseline. Respect the current task mode, compiled graph, active docs, and runtime selections before acting."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Before each action, check whether the step needs approval, whether a safer inspection path exists, and whether the current budget and runtime ownership allow the action."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("After each meaningful action, preserve unresolved items, restate the current state, and keep the next step aligned with the active handoff, checkpoints, and execution flow."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Route the next turn intentionally: continue if the goal is unchanged, compact when context is stale, handoff when another runtime owns the next step, and avoid unrelated work."),
                    ),
                    PromptModuleProfile(
                        module_id="compact",
                        stage="compaction",
                        ...
                        instruction_template=("When context grows, summarize durable decisions, active constraints, open questions, and artifact refs. Drop repetition and leave the next session executable."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Use previous-session summaries as compressed working memory. Reuse them to resume quickly, but re-check any fact that depends on current repository state."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Auto-allow safe inspection work, require approval for writes and side effects, and block actions that violate explicit policy or move outside the active repository and runtime boundaries."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Keep the current plan, unresolved blockers, checkpoints, and durable repository decisions visible. Prefer compact working memory over replaying stale transcript content."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Before claiming completion, review the requested behavior, changed or inspected surfaces, verification results, and obvious regressions. Escalate to review paths when the task warrants it."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("If the session is in coordinator mode, decompose the work into bounded worker tasks, keep the central plan current, synthesize worker results, and avoid delegating urgent blocking decisions."),
                    ),
                    PromptModuleProfile(
                        ...
                        instruction_template=("Choose the smallest adequate model path for the current turn's reasoning, context, and modality needs. Prefer stable fallbacks over brittle premium-only assumptions."),
                    ),
                ],
                source_alignment=[
