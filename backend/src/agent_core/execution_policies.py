...
from src.agent_core.roles import is_management_role, split_execution_roles

if TYPE_CHECKING:
    from src.task_workspaces.contracts import AgentHandle, TaskWorkspace
...
def get_execution_retry_budget(workspace: TaskWorkspace) -> int:
    ...
    return max(1, min(iterations, 10))
...
def evaluate_task_outcome(
    ...
    if expected_keywords:
        ...
        if _is_generic_fallback_banner(normalized_output):
            ...
            if goal_tokens and not any(token in normalized_output for token in goal_tokens):
...
_STOP_TOKENS = {
    ...
    "的", "了", "和", "与", "或", "及", "以及", "对", "向", "为", "对于", "关于", "给出",
    "请", "列出", "包含", "包括", "提供", "分析", "研究", "查询", "查找", "搜索", "检索",
    "今天", "明天", "后天", "未来", "目前", "最新", "当前", "实时", "数据", "结果", "url",
    "urls", "每", "日", "天", "地", "处", "位", "个", "条", "份", "种",
    # English stop-ish tokens.
    "the", "and", "or", "of", "to", "in", "on", "for", "at", "by", "with",
    "from", "a", "an", "is", "are", "be", "please", "provide", "list", "show",
    "current", "latest", "today", "tomorrow", "next", "day", "days", "url",
    "source", "sources",
}
