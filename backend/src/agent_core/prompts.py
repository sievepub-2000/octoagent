...
if TYPE_CHECKING:
    from src.task_workspaces.contracts import AgentHandle, AgentMessage, TaskWorkspace
...
def build_lead_agent_prompt(
    ...
    if attempt == 1:
        if is_multi_agent:
            return (
                f"你是主代理（协调者），必须通过以下专项子agents完成任务：{sub_desc}。\n\n"
                "## 执行规则\n"
    ...
    if is_multi_agent:
        return (
            retry_header
            + f"然后重新分配并督导子agents（{sub_desc}）完成任务。\n"
            "注意：你不能跳过子agent直接得出结论；最终结果必须基于子agent真实输出。\n\n"
            f"原始任务目标：{goal}"
        )
    return retry_header + "然后重新执行任务，确保结果完整可验证。\n\n" + f"原始任务目标：{goal}"
...
def build_multi_agent_kickoff_prompt(
    ...
    if attempt > 1:
        ...
        retry_context = (
            f"\n## 纠错上下文\n"
            f"- 上一轮失败原因：{last_failure_reason or '结果不完整'}\n"
            f"- 上一轮结果摘要：{prev_summary or '（无）'}\n"
        )
    return (
        f"你是 {workspace.mode} 工作流的主协调代理，负责制定分工计划并最终汇总各子代理的输出。\n\n"
        f"任务目标：{goal}\n\n"
...
def build_worker_prompt(
    ...
    prior_outputs = "\n\n".join(
        f"[{name}]\n{output}" for name, output in shared_outputs.items() if output.strip()
    ) or "（暂无其他 worker 输出）"
    return (
        f"你是 {agent.name}，职责是：{agent.task_scope or agent.role}。\n\n"
        f"总任务：{goal}\n\n"
...
def build_reviewer_prompt(goal: str, worker_outputs: dict[str, str]) -> str:
    ...
    return (
        f"你是 Review Agent，负责核查以下任务是否真正完成：{goal}\n\n"
        f"待核查 worker 输出：\n{combined}\n\n"
        f"请输出：完成项、缺失项、可疑结论、是否可以交付。若不能交付，必须明确拒绝。"
    )
...
def build_worker_timeout_analysis_prompt(
    ...
    prior_block = "\n\n".join(
        f"[{name}]\n{output}" for name, output in prior_worker_outputs.items() if output.strip()
    ) or "（暂无其他 worker 输出）"
    return (
        f"子代理 **{agent_name}**（职责：{agent_task_scope}）在执行过程中超时（超过 {timeout_seconds}s 未返回）。\n"
...
def build_worker_takeover_prompt(
    ...
    analyses_block = "\n\n".join(
        f"第{i + 1}次超时分析：{a}" for i, a in enumerate(timeout_analyses)
    )
    return (
        f"子代理 **{agent_name}**（职责：{agent_task_scope}）已连续 3 次超时，无法继续由其执行。\n\n"
        f"总任务目标：{goal}\n\n"
        f"三次超时分析摘要：\n{analyses_block}\n\n"
        f"已完成的其他 worker 输出：\n{prior_block}\n\n"
...
def build_worker_self_analysis_prompt(
    ...
    return (
        f"## 子代理自我复盘（第 {attempt_index} 次尝试 / 阶段一）\n\n"
        f"任务目标：{goal}\n\n"
...
def build_worker_stage2_prompt(
    ...
    return (
        f"## 阶段二：按主代理优化方案执行（第 {attempt_index} 次尝试）\n\n"
        f"任务目标：{goal}\n\n"
