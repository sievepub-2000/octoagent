...
from .graphrag_store import BootstrapGraphRAGStore
...
class EmbeddedBootstrapRuntime:
    ...
    def graphrag_root(self) -> Path:
        configured = Path(self._config.graphrag_root)
        if configured.is_absolute():
            return configured
        if self._config.project_managed:
            return self.repo_root() / configured
    ...
    def graphrag_store(self) -> BootstrapGraphRAGStore:
        return BootstrapGraphRAGStore(self.graphrag_root())
    ...
    def status(self) -> dict[str, Any]:
        ...
        graphrag_store = self.graphrag_store()
        return {
            ...
            "graphrag_enabled": self._config.graphrag_enabled,
            "retrieval_backend": self._config.retrieval_backend,
            "use_for_embeddings": self._config.use_for_embeddings,
            "vector_store_path": str(self.vector_store_path()),
            "graphrag_root": str(self.graphrag_root()),
            "graphrag_input_dir": str(graphrag_store.input_dir),
            "graphrag_output_dir": str(graphrag_store.output_dir),
            "graphrag_cli_available": graphrag_store.cli_available(),
            "graphrag_initialized": graphrag_store.is_initialized(),
            "graphrag_index_ready": graphrag_store.has_index(),
            "graphrag_query_method": self._config.graphrag_query_method,
            ...
            "recommended_model": "Gemma 3 270M IT GGUF Q4_K_M",
            ...
            if not self.is_installed():
                if self._config.auto_download:
                    ...
                    raise FileNotFoundError(
                        f"Embedded model file not found at {self.model_path()}. Install it first."
                    )
            self._llm = Llama(
                ...
                n_batch=self._config.n_batch,
                embedding=self._config.use_for_embeddings,
                verbose=False,
            )
        return self._llm
    ...
    def generate_guide(
        ...
        *,
        ...
        if self._config.graphrag_enabled and self._config.retrieval_backend == "graphrag":
            try:
                graphrag_result = self.graphrag_store().query(
                    query_text=user_goal or "首次使用 OctoAgent 的本地部署与工作流引导",
                    method=self._config.graphrag_query_method,
                )
                if graphrag_result:
                    retrieved_items = [graphrag_result.response]
                    retrieved_context = graphrag_result.response
            except Exception:
                retrieved_context = ""
        ...
        if not retrieved_items and self._config.use_for_embeddings:
            try:
                query_embedding = self.embed_text(
                    user_goal or "首次使用 OctoAgent 的本地部署与工作流引导"
                )
                matches = []
    ...
    def sync_local_corpus(self) -> dict[str, int]:
        ...
        documents = []
        graphrag_documents: list[tuple[str, str]] = []
        for path in self._local_corpus_files():
            ...
            graphrag_documents.append((relative_source, content))
        ...
        if self._config.graphrag_enabled:
            graphrag_store = self.graphrag_store()
            graphrag_store.sync_documents(graphrag_documents)
            try:
                graphrag_store.ensure_project_initialized()
            except Exception:
                pass
        if documents:
