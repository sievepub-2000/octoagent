...
from src.config.extensions_config import ExtensionsConfig, HookStateConfig, get_extensions_config, reload_extensions_config
...
class HookCoreService:
    ...
    def register_webhook(
        ...
        *events* gets a listener that posts the payload as JSON.
        ...
        if parsed.scheme not in self._ALLOWED_WEBHOOK_SCHEMES:
            raise ValueError(
                f"Webhook URL scheme must be one of {self._ALLOWED_WEBHOOK_SCHEMES}, got {parsed.scheme!r}"
            )
    ...
    def _make_webhook_poster(registration: WebhookRegistration) -> HookListener:
        ...
        async def _post_webhook(payload: dict[str, Any]) -> None:
            ...
            if registration.secret:
                ...
                sig = hmac.new(
                    registration.secret.encode(), body_bytes, hashlib.sha256
                ).hexdigest()
    ...
    def dispatch(self, event: str, payload: dict[str, Any] | None = None) -> int:
        ...
        enabled_events = {
            binding.event for binding in self._bindings.values() if binding.enabled
        }
    ...
    def _bridge_to_channel(self, event: str, payload: dict[str, Any]) -> None:
        ...
        if channel_type_val is None:
            return
        try:
            from src.channel_sdk import ChannelEvent, ChannelEventType
