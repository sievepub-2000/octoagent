    ...
    def get_runtime_snapshot(self) -> ResearchRuntimeSnapshot:
        ...
        return ResearchRuntimeSnapshot(
            ...
            active_experiments=sum(
                1 for experiment in experiments if experiment.status in {"queued", "running"}
            ),
    ...
    def _seed_programs(self) -> list[ResearchInstructionProgram]:
        return [
            ResearchInstructionProgram(
                ...
                summary=(
                    "Run short experiment loops with explicit guardrails, reviewable diffs, "
                    "and metric-based keep-or-discard verdicts."
                ),
