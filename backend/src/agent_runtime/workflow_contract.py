...
class LangGraphWorkflowContractService:
    ...
    def _load(self) -> LangGraphWorkflowContractState:
        ...
        if not self._path.exists():
            ...
            return self._state
        self._state = LangGraphWorkflowContractState.model_validate_json(
    ...
    async def prune_remote_threads(
        ...
        *,
        ...
        remote_thread_ids = [
            thread_id for thread_id in thread_ids if _is_langgraph_remote_thread_id(thread_id)
        ]
        remote_thread_id_set = set(remote_thread_ids)
        skipped_thread_ids = [
            thread_id for thread_id in thread_ids if thread_id not in remote_thread_id_set
        ]
