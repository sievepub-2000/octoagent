...
from src.agent_runtime.langgraph_remote import normalize_remote_run_payload
from src.agent_runtime.workflow_contract import get_langgraph_workflow_contract_service
from src.query_engine import QueryOperationPlanRequest, get_query_engine_service
...
def _resolve_assistant_id(request: AgentExecutionRequest) -> str:
    return (
        _read_hint(request.assistant_id)
        or _workspace_hint(request.workspace_metadata, "langgraph_assistant_id")
...
class LangGraphRuntimeProvider:
    ...
    async def execute(self, request: AgentExecutionRequest) -> AgentExecutionResult:
        ...
        try:
            ...
            logger.warning(
                "LangGraph run timed out for task %s after %ss (thread=%s, assistant=%s)",
                request.task_id,
            ...
            raise RuntimeError(
                f"LangGraph execution timed out after {request.timeout_seconds}s "
                f"(thread={tid}, assistant={assistant_id})"
            ) from exc
            ...
            raise RuntimeError(
                f"LangGraph execution failed (thread={tid}, assistant={assistant_id}): {exc}"
            ) from exc
