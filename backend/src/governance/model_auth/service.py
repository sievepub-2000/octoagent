...
_PROVIDERS_YAML = Path(__file__).with_name("providers.yaml")


def _load_providers() -> dict[str, ProviderTemplate]:
    raw = yaml.safe_load(_PROVIDERS_YAML.read_text(encoding="utf-8"))
    return {key: ProviderTemplate(**value) for key, value in raw.items()}


PROVIDER_TEMPLATES = _load_providers()
