...
def test_oom_guard_cleans_at_85_without_stopping(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.runtime.oom_guard.get_memory_pressure_snapshot",
        lambda **_: _snapshot(86.0),
    )
    monkeypatch.setattr(
        "src.runtime.oom_guard.cleanup_memory",
        lambda snapshot, *, reason: cleanup_calls.append(reason) or {"reason": reason},
    )
    ...
    monkeypatch.setattr(
        "src.runtime.oom_guard.cancel_busy_langgraph_runs",
        _unexpected_cancel,
    )
...
def test_oom_guard_stops_at_90_and_reports_hardware(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.runtime.oom_guard.get_memory_pressure_snapshot",
        lambda **_: _snapshot(91.0),
    )
    monkeypatch.setattr(
        "src.runtime.oom_guard.cleanup_memory",
        lambda snapshot, *, reason: {
            "reason": reason,
    ...
    assert report["langgraph"]["cancelled_runs"] == [
