...
def _execution_module():
    return importlib.import_module("src.agents.middlewares.execution_middleware")


def _step_reflection_module():
    return importlib.import_module("src.agents.middlewares.step_reflection_middleware")
...
def test_closure_only_triggers_on_must_finalize_status():
    mod = _execution_module()
    assert mod._research_closure_active({"research_closure": {"status": "must_finalize"}}) is True
    ...
    for status in ("active", "finalized", "pending", "draft", ""):
        ...
        assert mod._research_closure_active(runtime) is False, f"closure should not trigger on status={status!r}"
...
def test_closure_helper_in_step_reflection_matches_execution_middleware():
    ...
    assert exec_mod._research_closure_active(closure_state["runtime"]) is True

    inactive_state = {"runtime": {"research_closure": {"status": "active"}}}
    assert step_mod._research_closure_active(inactive_state) is False
    assert exec_mod._research_closure_active(inactive_state["runtime"]) is False
