...
.PHONY: help config check install dev dev-daemon start start-daemon stop clean execution-worker smoke-real smoke-mock smoke-ui migrate-memory release-precheck operator-release sync-check sync-align sync-references docker-init docker-start docker-stop docker-logs docker-logs-frontend docker-logs-gateway
