# OctoAgent Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the OctoAgent backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `octoagent` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `octoagent` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `octoagent` namespace
     - Create/read/delete Services in the `octoagent` namespace
     - Read Namespaces (to create `octoagent` if missing)
...
docker compose -p octoagent-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
```bash
# Health check
curl http://localhost:19808/health

# Create a sandbox (via provisioner container for internal DNS)
docker exec octoagent-provisioner curl -X POST http://localhost:19808/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec octoagent-provisioner curl http://localhost:19808/api/sandboxes/test-001

# List all sandboxes
docker exec octoagent-provisioner curl http://localhost:19808/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n octoagent -l sandbox-id=test-001

# Delete sandbox
docker exec octoagent-provisioner curl -X DELETE http://localhost:19808/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec octoagent-provisioner curl -s http://localhost:19808/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec octoagent-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec octoagent-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.octoagent/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n octoagent`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n octoagent`
...
3. **Network Isolation**: Sandbox Pods run in the `octoagent` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
