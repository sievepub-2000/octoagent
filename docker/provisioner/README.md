...
docker exec octoagent-provisioner curl -X POST http://localhost:19888/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec octoagent-provisioner curl http://localhost:19888/api/sandboxes/test-001

# List all sandboxes
docker exec octoagent-provisioner curl http://localhost:19888/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n octoagent -l sandbox-id=test-001

# Delete sandbox
docker exec octoagent-provisioner curl -X DELETE http://localhost:19888/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec octoagent-provisioner curl -s http://localhost:19888/api/sandboxes/test-001 | jq -r .sandbox_url)
