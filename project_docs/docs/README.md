...
After the P9 governance pass, the repository baseline uses bounded uv lock verification, backend compile, backend ruff, doctor/API contract smoke, workflow/LangGraph lifecycle smoke, long-running soak sampling, frontend lint, frontend typecheck, frontend build, release precheck, and targeted WebUI smoke validation. Capability binding includes policy import/export/precheck, workflow runtime sustainability includes lifecycle audit actions, and operator governance now covers runtime alerts, distributed execution, and tenant controls.
