...
- **嵌入式模型**：内置 Bootstrap 模型用于冷启动、GraphRAG 检索

## 2. 系统架构总览

```
┌──────────────── 客户端层 ────────────────┐
│  Next.js 16 WebUI (Port 19886)           │
│  ├── Workspace 路由系统                   │
...
| 服务 | 端口 | 说明 |
|------|------|------|
| Next.js Frontend | 19886 | WebUI |
| FastAPI Gateway | 19882 | REST API |
| LangGraph Runtime | 19884 | Agent 运行时 |
| Nginx (Docker) | 19880 | 反向代理 / 统一入口 / `/auth/register` 注册登录页 |
...
```bash
# 前端
cd frontend && pnpm exec next dev --turbo --hostname 127.0.0.1 --port 19886
# 网关
cd backend && .venv/bin/python -m uvicorn src.gateway.app:app --host 127.0.0.1 --port 19882
# LangGraph
cd backend && .venv/bin/python -m langgraph_cli dev --no-browser --allow-blocking --host 127.0.0.1 --port 19884
```
