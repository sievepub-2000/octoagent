...
- `workspace/default/memory*.json` and `workspace/default/agents/*/memory.json` are runtime state and must stay out of Git.
...
`scripts/check_legacy_paths.py` fails CI if source-controlled files reintroduce known stale runtime path patterns or tracked runtime memory files.
