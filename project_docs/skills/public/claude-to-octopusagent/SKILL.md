...
Communicate with a running OctopusAgent instance via its HTTP API. OctopusAgent is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

OctopusAgent exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$OCTOPUSAGENT_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$OCTOPUSAGENT_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `OCTOPUSAGENT_URL`          | `http://localhost:19880`                  | Unified proxy base URL             |
| `OCTOPUSAGENT_GATEWAY_URL`  | `${OCTOPUSAGENT_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `OCTOPUSAGENT_LANGGRAPH_URL`| `${OCTOPUSAGENT_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
OCTOPUSAGENT_URL="${OCTOPUSAGENT_URL:-http://localhost:19880}"
OCTOPUSAGENT_GATEWAY_URL="${OCTOPUSAGENT_GATEWAY_URL:-$OCTOPUSAGENT_URL}"
OCTOPUSAGENT_LANGGRAPH_URL="${OCTOPUSAGENT_LANGGRAPH_URL:-$OCTOPUSAGENT_URL/api/langgraph}"
```
...
Verify OctopusAgent is running:

```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$OCTOPUSAGENT_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$OCTOPUSAGENT_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$OCTOPUSAGENT_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$OCTOPUSAGENT_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$OCTOPUSAGENT_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$OCTOPUSAGENT_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$OCTOPUSAGENT_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-octopusagent/scripts/chat.sh "Your question here"
```
...
- If health check fails, OctopusAgent is not running. Inform the user they need to start it.
