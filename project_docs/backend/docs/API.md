...
This document provides a complete reference for the OctopusAgent backend APIs.

## Overview

OctopusAgent backend exposes two sets of APIs:
  ...
  "files": [
    {
      ...
      "size": 1234567,
      "path": ".octopusagent/threads/abc123/user-data/uploads/document.pdf",
      "virtual_path": "/mnt/user-data/uploads/document.pdf",
      "artifact_url": "/api/threads/abc123/artifacts/mnt/user-data/uploads/document.pdf",
      "markdown_file": "document.md",
      "markdown_path": ".octopusagent/threads/abc123/user-data/uploads/document.md",
...
Currently, OctopusAgent does not implement authentication. All APIs are accessible without credentials.

Note: This is about OctopusAgent API authentication. MCP outbound connections can still use OAuth for configured HTTP/SSE MCP servers.
...
```
ws://localhost:19880/api/langgraph/threads/{thread_id}/runs/stream
```
...
client = get_client(url="http://localhost:19880/api/langgraph")
...
```bash
# List models
curl http://localhost:19880/api/models

# Get MCP config
curl http://localhost:19880/api/mcp/config

# Upload file
curl -X POST http://localhost:19880/api/threads/abc123/uploads \
  -F "files=@document.pdf"

# Enable skill
curl -X POST http://localhost:19880/api/skills/pdf-processing/enable

# Create thread and run agent
curl -X POST http://localhost:19880/api/langgraph/threads \
  -H "Content-Type: application/json" \
  -d '{}'

curl -X POST http://localhost:19880/api/langgraph/threads/abc123/runs \
  -H "Content-Type: application/json" \
  -d '{
    "input": {"messages": [{"role": "user", "content": "Hello"}]},
    "config": {"configurable": {"model_name": "gpt-4"}}
