...
Quick setup instructions for OctopusAgent.

## Configuration Setup

OctopusAgent uses a YAML configuration file that should be placed in the **project root directory**.

### Steps

1. **Navigate to project root**:
   ```bash
   cd /path/to/octopusagent
   ```
...
- **Location**: `config.yaml` should be in `octopusagent/` (project root), not `octopusagent/backend/`
...
1. `OCTO_AGENT_CONFIG_PATH` environment variable (if set)
2. `backend/config.yaml` (current directory when running from backend/)
3. `octopusagent/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`octopusagent/config.yaml`).
