...
4. **Access the application**:
   - Web Interface: http://localhost:19880
   - API Gateway: http://localhost:19880/api/*
   - LangGraph: http://localhost:19880/api/langgraph/*
...
make docker-init
# Start Docker services (mode-aware, localhost:19880)
make docker-start
...
Docker Compose (`octoagent-dev`)
  ├→ nginx (port 19880) ← Reverse proxy
  ...
  ├→ api (port 19832) ← Gateway API with hot-reload
   ...
   └→ provisioner (optional, port 19833) ← Started only in provisioner/K8s sandbox mode
```
...
- Unified entry point on port 19880
...
Browser
  ↓
Nginx (port 19880) ← Unified entry point
